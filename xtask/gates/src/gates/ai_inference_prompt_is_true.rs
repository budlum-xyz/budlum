...
//!
//! The prompt in `crates/ai-inference/crates/ai-inference-core/src/system_prompt.rs` tells
...
//! it: no `ai-inference-core` module produces an answer, so [`check_answer_surface`]
...
//! themselves. The workspace tests inside `ai-inference-core` check that the prompt
...
//!
//! # Why this is not the `ai_inference-reads` gate
//!
//! `ai_inference-reads` guards the perception module: no generating variant, four
...
const PROMPT_PATH: &str = "crates/ai-inference/crates/ai-inference-core/src/system_prompt.rs";
...
const BACKED_PROMISES: &[(&str, &str, &str, &str)] = &[
    ...
    (
        ...
        "crates/ai-inference/crates/ai-inference-serve/src/config.rs",
    ...
    (
        ...
        "crates/ai-inference/crates/ai-inference-knowledge/src/redact.rs",
    ...
    (
        ...
        "crates/ai-inference/crates/ai-inference-data/src/source.rs",
...
pub fn check_with_effort(
    ...
    // beside the prompt, in `ai-inference-core`; duplicating the phrase list here
...
const MASK_SITES: &[(&str, &str, &str)] = &[
    (
        "Masking happens before storage, not after",
        "crates/ai-inference/crates/ai-inference-knowledge/src/cache.rs",
        "redact_model_strings(",
    ),
    (
        "Masking happens before storage, not after",
        "crates/ai-inference/crates/ai-inference-knowledge/src/memory.rs",
        "redact_model_strings(",
    ),
    (
        "treat the credential as unreadable",
        "crates/ai-inference/crates/ai-inference-knowledge/src/chunk.rs",
        "redact_text(",
    ),
];
...
/// Refuse an answer-producing surface in `ai-inference-core` that does not mask.
///
/// The prompt promises no key reaches an output. Today no `ai-inference-core` module
...
fn extract_prompt_text(prompt_source: &str) -> Result<&str, String> {
    let anchor = "pub const AI_INFERENCE_SYSTEM_PROMPT: &str = r#\"";
    let start = prompt_source
        .find(anchor)
        .ok_or_else(|| format!("AI_INFERENCE_SYSTEM_PROMPT is gone from {PROMPT_PATH}"))?
        + anchor.len();
    let rest = &prompt_source[start..];
    let end = rest
        .find("\"#")
        .ok_or_else(|| String::from("AI_INFERENCE_SYSTEM_PROMPT has no closing delimiter"))?;
    let text = &rest[..end];
    if text.trim().is_empty() {
        return Err(String::from(
            "AI_INFERENCE_SYSTEM_PROMPT is empty: the model would be handed no instructions at all.",
        ));
    }
    Ok(text)
}
...
pub fn run(root: &Path) -> Result<String, String> {
    ...
    let core = root.join("crates/ai-inference/crates/ai-inference-core/src");
    ...
    if core_files.len() < 3 {
        ...
        let body =
            "ai-inference-core has more than three modules, so a scan that saw this many read \
    ...
    Ok(format!(
        "the the AI inference layer system prompt agrees with the tree: {} claims checked \
         ...
         {answering} answering surface(s) in ai-inference-core each masked)",
        checked + backed,
        LIMITS.len(),
        EFFORT_FIGURES.len(),
        REQUIRED_CLAIMS.len()
...
";
    let good_prompt = "\
pub const AI_INFERENCE_SYSTEM_PROMPT: &str = r#\"
