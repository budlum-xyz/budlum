//! the AI inference layer's tier story and its dataset hash have to stay checkable.
//!
//! Gate code: `K-AI-VERIFIABLE-INFERENCE-MESH`. A finding or a document that names this code resolves here.
//!
//! The invention text promises a ZK-checked inference path for the AI inference layer. Two
...
//! `ai-inference-data` still exposes `verify_sha256` for the provenance hash. A
...
pub fn run(root: &Path) -> Result<String, String> {
    let tier = read(
        root,
        "crates/ai-inference/crates/ai-inference-core/src/tier.rs",
    )?;
    let data = read(
        root,
        "crates/ai-inference/crates/ai-inference-data/src/verify.rs",
    )
    .unwrap_or_default();
    ...
    if data.contains("pub fn verify_sha256") {
        ...
        problems.push(
            "`ai-inference-data` no longer exposes `verify_sha256`. Provenance of a dataset entry is \
    ...
    Ok(format!(
        "the AI inference layer inference surface OK: {checked} checks, {} tier variants with no numeric \
...
pub fn self_test() -> Result<String, String> {
    ...
    std::fs::create_dir_all(dir.join("crates/ai-inference/crates/ai-inference-core/src"))
        .map_err(|e| e.to_string())?;
    std::fs::create_dir_all(dir.join("crates/ai-inference/crates/ai-inference-data/src"))
        .map_err(|e| e.to_string())?;
    ...
    std::fs::write(
        dir.join("crates/ai-inference/crates/ai-inference-core/src/tier.rs"),
        good,
    )
    .map_err(|e| e.to_string())?;
    std::fs::write(
        dir.join("crates/ai-inference/crates/ai-inference-data/src/verify.rs"),
    ...
    let _ = std::fs::remove_dir_all(&dir);
    Ok(String::from(
        "the AI inference layer canary OK (clean tiers PASS; a numeric tier label and a missing dataset \
         verifier each FAIL).",
    ))
}
