//! Agent reads; it does not generate.
//!
//! Ported from `scripts/check-agent-reads-but-does-not-generate.sh`. The
...
fn code_of(root: &Path) -> Result<String, String> {
    let f = root.join("src/agent/perception.rs");
    if !f.is_file() {
...
pub fn run(root: &Path) -> Result<String, String> {
                ...
                || t.contains("Synthesis"))
        {
            return Err(String::from(
                "PerceptionKind gained a generating variant.\n  \
                 Agent reads; it does not produce images or video. Generation needs its own\n  \
    ...
    Ok(String::from(
        "Agent-reads gate OK: no generation surface, per-modality budgets, fail-closed default, decoder boundary.",
    ))
}
...
pub fn self_test() -> Result<String, String> {
    ...
    let _ = std::fs::create_dir_all(dir.join("src/agent"));
    ...
    std::fs::write(dir.join("src/agent/perception.rs"), good).map_err(|e| e.to_string())?;
    if run(&dir).is_err() {
    ...
    std::fs::write(dir.join("src/agent/perception.rs"), bad).map_err(|e| e.to_string())?;
    if run(&dir).is_ok() {
        let _ = std::fs::remove_dir_all(&dir);
    ...
    let _ = std::fs::remove_dir_all(&dir);
    Ok(String::from(
        "agent canary OK (correct PASSes, a generative variant FAILs).",
    ))
}
