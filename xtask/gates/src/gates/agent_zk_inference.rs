//! Agent's tier story and its dataset hash have to stay checkable.
//!
//! Gate code: `K-AI-VERIFIABLE-INFERENCE-MESH`. A finding or a document that names this code resolves here.
//!
//! The invention text promises a ZK-checked inference path for Agent. Two
...
//! `agent-data` still exposes `verify_sha256` for the provenance hash. A
...
pub fn run(root: &Path) -> Result<String, String> {
    let tier = read(root, "crates/agent/crates/agent-core/src/tier.rs")?;
    let data = read(root, "crates/agent/crates/agent-data/src/verify.rs").unwrap_or_default();
    ...
    for v in &variants {
        ...
        if label.chars().any(|c| c.is_ascii_digit()) {
            problems.push(format!(
                "variant `{v}` names a number. Multiplier tier labels do not exist in Agent: \
    ...
    if served.contains("\"agent-") && served.contains("self") {
        checked += 1;
    } else {
        problems.push(
    ...
    if data.contains("pub fn verify_sha256") {
        ...
        problems.push(
            "`agent-data` no longer exposes `verify_sha256`. Provenance of a dataset entry is \
    ...
    Ok(format!(
        "Agent inference surface OK: {checked} checks, {} tier variants with no numeric \
...
pub fn self_test() -> Result<String, String> {
    let nanos = std::time::SystemTime::now()
        ...
        .subsec_nanos();
    let dir =
        std::env::temp_dir().join(format!("budlum-gates-agent-{}-{nanos}", std::process::id()));
    std::fs::create_dir_all(dir.join("crates/agent/crates/agent-core/src"))
        .map_err(|e| e.to_string())?;
    std::fs::create_dir_all(dir.join("crates/agent/crates/agent-data/src"))
        .map_err(|e| e.to_string())?;
    let good = "pub enum ModelTier {\n    /// Everyday use.\n    Light,\n    /// Highest capacity.\n    Normal,\n}\n\nimpl ModelTier {\n    pub fn served_model_name(self, version: &str) -> String {\n        format!(\"agent-{}-{version}\", self.name())\n    }\n}\n\n#[cfg(test)]\nmod tests {\n    fn served_names_follow_tier_naming() {}\n    fn tier_names_contain_no_multiplier_labels() {}\n}\n";
    std::fs::write(dir.join("crates/agent/crates/agent-core/src/tier.rs"), good)
        .map_err(|e| e.to_string())?;
    std::fs::write(
        dir.join("crates/agent/crates/agent-data/src/verify.rs"),
    ...
    std::fs::write(dir.join("crates/agent/crates/agent-core/src/tier.rs"), bad)
        .map_err(|e| e.to_string())?;
    if run(&dir).is_ok() {
        let _ = std::fs::remove_dir_all(&dir);
    ...
    let _ = std::fs::remove_dir_all(&dir);
    Ok(String::from(
        "Agent canary OK (clean tiers PASS; a numeric tier label and a missing dataset \
         verifier each FAIL).",
    ))
}
