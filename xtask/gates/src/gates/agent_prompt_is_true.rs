...
//!
//! The prompt in `crates/agent/crates/agent-core/src/system_prompt.rs` tells
...
//! `src/agent/perception.rs`, the channels are the variants of `SourceKind`,
...
//! it: no `agent-core` module produces an answer, so [`check_answer_surface`]
...
//! themselves. The workspace tests inside `agent-core` check that the prompt
...
//!
//! # Why this is not the `agent-reads` gate
//!
//! `agent-reads` guards the perception module: no generating variant, four
...
const PROMPT_PATH: &str = "crates/agent/crates/agent-core/src/system_prompt.rs";

/// Where the chain-side ceilings live.
const PERCEPTION_PATH: &str = "src/agent/perception.rs";
...
const EFFORT_PATH: &str = "src/agent/effort.rs";
...
const BACKED_PROMISES: &[(&str, &str, &str, &str)] = &[
    ...
    (
        ...
        "crates/agent/crates/agent-serve/src/config.rs",
    ...
    (
        ...
        "crates/agent/crates/agent-knowledge/src/redact.rs",
    ...
    (
        ...
        "crates/agent/crates/agent-data/src/source.rs",
...
pub fn check_with_effort(
    ...
    // beside the prompt, in `agent-core`; duplicating the phrase list here
            ...
            if !negated {
                return Err(format!(
                    "the prompt offers media generation: `{marker}` in\n    {}\n  \
                     Agent reads. A generating surface needs its own economics, its own abuse \
...
const MASK_SITES: &[(&str, &str, &str)] = &[
    (
        "Masking happens before storage, not after",
        "crates/agent/crates/agent-knowledge/src/cache.rs",
        "redact_model_strings(",
    ),
    (
        "Masking happens before storage, not after",
        "crates/agent/crates/agent-knowledge/src/memory.rs",
        "redact_model_strings(",
    ),
    (
        "treat the credential as unreadable",
        "crates/agent/crates/agent-knowledge/src/chunk.rs",
        "redact_text(",
    ),
];
...
/// Refuse an answer-producing surface in `agent-core` that does not mask.
///
/// The prompt promises no key reaches an output. Today no `agent-core` module
...
fn extract_prompt_text(prompt_source: &str) -> Result<&str, String> {
    let anchor = "pub const AGENT_SYSTEM_PROMPT: &str = r#\"";
    let start = prompt_source
        .find(anchor)
        .ok_or_else(|| format!("AGENT_SYSTEM_PROMPT is gone from {PROMPT_PATH}"))?
        + anchor.len();
    let rest = &prompt_source[start..];
    let end = rest
        .find("\"#")
        .ok_or_else(|| String::from("AGENT_SYSTEM_PROMPT has no closing delimiter"))?;
    let text = &rest[..end];
    if text.trim().is_empty() {
        return Err(String::from(
            "AGENT_SYSTEM_PROMPT is empty: the model would be handed no instructions at all.",
        ));
    }
    Ok(text)
}
...
pub fn run(root: &Path) -> Result<String, String> {
    ...
    let core = root.join("crates/agent/crates/agent-core/src");
    ...
    if core_files.len() < 3 {
        ...
        let body = "agent-core has more than three modules, so a scan that saw this many read \
    ...
    Ok(format!(
        "the Agent system prompt agrees with the tree: {} claims checked \
         ...
         {answering} answering surface(s) in agent-core each masked)",
        checked + backed,
        LIMITS.len(),
        EFFORT_FIGURES.len(),
        REQUIRED_CLAIMS.len()
...
";
    ...
    Ok(String::from("agent-prompt-is-true: 18 canaries"))
}
