...
const FORBIDDEN: &[(&str, &str)] = &[
    (
        ...
        "an on-device MoE engine. What we took was the idea of a placement \
         ...
         `agent-serve/src/residency.rs` under our own terms.",
    ),
    (
        "jcode",
    ...
    (
        ...
        "a corpus of published system prompts, read while writing \
         `agent-core/src/system_prompt.rs`. Our prompt states our own system's \
         behaviour and is checked against the tree by `agent-prompt-is-true`.",
    ),
];
