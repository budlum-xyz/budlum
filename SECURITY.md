...
| Kani | bond arithmetic: no slash exceeds its bond | gate |
...
The replacement is real. `src/registry/kani_proofs.rs` carries five harnesses
over the slash penalty computation, and `.github/workflows/extra-tooling.yml`
runs them on every pull request against a pinned Kani. The properties proved are
that a penalty never exceeds the bond it is taken from, that the
`saturating_sub` in `slash_role_only` never masks an underflow, that the 0% and
100% ratios are exact, and that the penalty is monotonic in the ratio. A fifth
harness drops the `ratio <= FIXED_POINT_SCALE` precondition and shows the bound
would break without it, which records `RegistryParams::validate` as
...
harnesses written against extracted, bounded logic rather than the call sites.
