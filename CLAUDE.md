...
> `budlum-core` (ağ / L1) ve Task 0.37'te aynı repoya alınan `budzero/`
...
- RPC API: Relayer modeli netleştiği için (permissionless) RPC uç noktaları
  ...
  sorgulama uçları dahil. **Task 0.38, Görev 5 — B.U.D. storage RPC yüzeyi
...
- **Monorepo (Task 0.37):** BudZero/BudZKVM kaynakları `budzero/` altındadır.
...
- **B.U.D. (Task 0.38):** `ConsensusKind::StorageAttestation(StorageDomainParams)`
  yeni enum varyantı (vizyon §8.1, Custom(String) DEĞİL). Muhasebe Görev 1 =
  yalnızca domain kaydı; Görev 2 = content addressing + manifest;
  Görev 5 = deal/challenge ekonomisi. **`RetrievalChallenge` interim'dir** —
  Görev 3 (gerçek Proof-of-Storage) BudZKVM `VerifyMerkle` 64-depth
...
- `src/registry/role.rs` — generic `RoleId` (hard-coded enum değil). Bilinen
  ...
  `STORAGE_OPERATOR` (RoleId 5) **Task 0.38, Görev 1**: permissionless, stake ile
...
- `src/domain/storage_params.rs` — **Task 0.38, Görev 1 (B.U.D.)**:
...
- `src/domain/storage_deal.rs` — **Task 0.38, Görev 5 (B.U.D. deal/challenge
  ...
  saklayarak testi geçebilir. Tam kanıt (Görev 3, vision §8.3) BudZKVM
  ...
  admin/pause hook'u YOK — data-sovereignty kuralı (Task 0.39 plan §0.5).
- `src/storage/content_id.rs` — **Task 0.38, Görev 2 (B.U.D. content
  addressing)**: `ContentId` (32-byte domain-tagged SHA-256).
  `of_subrange` byte-range hash (interim) — gerçek proof DEĞİL.
- `src/storage/manifest.rs` — **Task 0.38, Görev 2**: `ContentManifest` +
...
- `src/domain/finality_adapter.rs` — domain finality doğrulayıcıları. Güvenlik
  durumu (Task 0.08-7 sonrası — TÜM aile gerçek doğrulama yapıyor): PoS gerçek BLS
  cert doğrular (`cert.verify`); **BFT** aynı desende gerçek BLS cert doğrular
  (self-reported signer_count KALDIRILDI, Task 0.10); **PoW** proof'u commitment'a
  bağlar (`declared_head_hash == commitment.domain_block_hash`) +
  declared_cumulative_work iç tutarlılık + min-work eşiği (Task 0.10; tam
  light-client DEĞİL, o ayrı iş); ZK `ProofClaimRegistry` üzerinden gerçek STARK
  (Task 0.08); **PoA** gerçek ed25519 imza kümesi doğrular (`FinalityProof::PoA` artık
  `authorities: Vec<Address>` + `signatures: Vec<PoAAuthoritySignature>` taşır,
  `poa_commit_signing_message` ile commitment'a bağlı, SAYI-bazlı quorum — Task 0.12).
  KRİTİK: PoA doğrulaması stake mekanizmasına/permissionless registry'ye
  BAĞLANMADI; PoA kendi ayrı stake-siz ed25519 modelini kullanır (Task 0-2
  izolasyonu korunur). PoA'da stake YOK (bilinçli). Yeni proof alanı eklerken
  `#[serde(default)]` kullan (geriye uyumluluk).
- `src/tokenomics/mod.rs` — $BUD tokenomik (Task 0.14). Arz 100M, **6 ondalık**
  ...
  ENTEGRASYON (Task 0.14b — gerçek zincir akışına bağlı): (a) genesis:
  ...
  `team_vesting` DÜZELTİLDİ (Task 0.16): artık snapshot'a girip çıkıyor.

- State persistence (Task 0.16): `StateSnapshotV2` `schema_version=3` ile `registry`,
...
- Sessiz-hata sertleştirmesi (Task 0.32): serialize-into-hash/persistence/network
...
- Liveness → epoch akışı (Task 0.30): `Blockchain::maybe_observe_liveness_on_epoch_close`
...
- Liveness DISPATCH (Task 0.34): `maybe_observe_liveness_on_epoch_close` artık
  ...
  * `false` (VARSAYILAN) → Task 0.30 `observe_liveness_epoch` = rapor + `tracing`
  ...
  `true` yapınca aktifleşir (Task 0.30 "önce gözlemle, canlı/testnet'te doğrula,
  sonra aç" kuralı). `record_liveness_epoch` mekanizması Task 0.04'ten beri var+test
  edilmişti; Task 0.34 sadece canlı akıştan erişilebilir kıldı. Kademeli ceza
  (liveness'te jail yok) ayrı/gelecek tur — `slash()`'a dokunulmadı.
- Adversarial finality testleri (Task 0.36-14): `src/tests/finality_adversarial.rs`.
...
- Aggregator sağlamlaştırması (Task 0.38, `FinalityAggregator`):
...
- Slashing kalıcılığı + geçmiş (Task 0.40, Görev 1): `PermissionlessRegistry`'ye
...
- Tekrarlı geçersiz-imza slashing (Task 0.40, Görev 2): yeni `InvalidVoteTracker`
...
- Prover → L1 köprüsü: `Blockchain::submit_zk_proof(ZkProofSubmission)`. Model B
  ...
  ödül; gönderim kapısı DEĞİL — Task 0.04 relayer kapısıyla karıştırma).
