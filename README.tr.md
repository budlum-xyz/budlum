...
flowchart TB
  ...
  GBH --> AI["Agent yapay zeka katmanı<br/>model kaydı · hesaplama teminatı"]
```
...
2. **Agent derecelendirme döngüsü.** `agent-ops tune data.jsonl` veri seti
   etiketini (hedef model, örnek sayısı) doğrular, müfredatın derecelendirme
   seti özetini plana iliştirip bağı kontrol eder; `agent-ops eval data.jsonl
   ...
   `agent-serve`, yerleşim planı operatörün `disk_budget_bytes` payından
...
| **Agent** | [`src/agent/`](src/agent) | Kapalı devre yapay zeka katmanı: model kayıt defteri, operatör hesaplama teminatı, çaba kademeleri, Pollen ile kapılanan veri erişimi, algı beyanları (V3), SocialFi çıktı köprüsü. Zincir dışı çalışma alanı: [`crates/agent/`](crates/agent) |
...
| Yol | İçerik |
| --- | --- |
| [`crates/`](crates) | Bağımsız çalışma alanları: wallet-core, budscan, agent, note-packing |
...
SocialFi ilkelleri · Agent yapay zeka çıkarım katmanı · EVM zincir adaptörü
