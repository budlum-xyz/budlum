...
```
[ On chain - budlum/main/src/agent/ ]        (exists; this repo does not touch it)
   model registry - operator compute bond - Pollen - B.U.D. - SocialFi
                        ^ hash and reference matching
[ This repo - the off-chain skeleton ]
   agent-core   : ModelId (a 32-byte hash, mirroring the AiModelId form),
                  dataset types, LoRaManifest
   agent-data   : closed-loop source checking over 3 channels, the record
                  format, fail-closed hash verification
   agent-serve  : the vLLM and SGLang bridge; the weight name is preserved and
                  the served name is ours
   agent-tune   : TunePlan (LoRA in BF16 or FP16, with no FP4), the output hash
                  lock
   agent-ops    : the CLI skeleton
```
...
1. **Closed loop:** there is no path that reads outside data; every set enters
   ...
   verification.
2. **Fail closed:** an unverified hash is a refusal, and `agent-data::verify`
...
- `agent-data`: real SHA-256 verification (`verify_sha256`, `content_id_of`),
  JSONL records built on serde_json, and a draft structural chat template.
- `agent-serve`: the tier naming (`agent-light-*` and `agent-normal-*`), the
  multiplier label check, and a fail-closed chain RPC draft
  (`chain::NotConnected`).
- `agent-core`: the `ModelTier` type, light or normal; `ModelSpec` carries the
  tier.

## The system prompt, 2026-08-25

The prompt handed to a model before the first user message lives in
`agent-core/src/system_prompt.rs` as a `const`, not in a configuration file.
...
- `agent-core` tests assert the prompt is internally consistent - the ceilings
  ...
  proven.
- The `agent-prompt-is-true` gate checks the half those tests cannot see: that
  the numbers the prompt tells a user are the numbers `src/agent/perception.rs`
  actually admits by. It fails in both directions, because which of the two is
  wrong is a judgement a gate should not make.

The prompt states why Agent reads and does not generate, at length and with the
...
`agent-prompt-is-true` therefore carries a table of (sentence, file, symbol)
...
`agent-serve/src/residency.rs` plans where each part of a model lives on the
