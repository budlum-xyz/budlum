...
> from the separate `budlum-xyz/agent` repository. The chain side lives in
> `src/agent/` and `src/ai/`; the off-chain skeleton stands in this `agent/`
> directory as its own Cargo workspace, in the same pattern as budzero. All
> Agent work is collected into a single pull request.

- **The on-chain side** is in the budlum main repository, under `src/agent/`:
...
- **The closed-loop principle:** Agent reads only data that carries a Pollen
...
- **The base model:** the DeepSeek V4 family, with MIT weights.
  ...
  first fine-tune.
- **Tier naming, 2026-08-13:** DeepSeek's variant names are not used in Agent.
  The Flash-based tier is served as **`agent-light`** and the Pro-based tier as
  **`agent-normal`**. Multiplier labels such as 0.5x or 10x do not exist in
  Agent, and the check lives in the code, at
  `agent-serve::config::assert_served_name_is_ours`.
- **Attribution policy:** the "DeepSeek to Agent" rename is done only in our
...
| Crate | Job |
|---|---|
| `agent-core` | The model identity, the dataset types and the LoRA manifest; mirror types, per decision K3 |
| `agent-data` | Closed-loop source checking, the record format, fail-closed verification |
| `agent-serve` | The vLLM and SGLang bridge configuration; the weight name is preserved and the served name is ours |
| `agent-tune` | The training plan, LoRA in BF16 or FP16, with FP4 absent from the type system, plus the output hash lock |
| `agent-ops` | The operator CLI skeleton: register, bond, serve, tune, status |
