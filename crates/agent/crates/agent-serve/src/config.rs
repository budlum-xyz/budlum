...
use agent_core::tier::ModelTier;

use crate::residency::{
    DeviceBudget, Placement, PlanError, ResidencyPlan, SemanticProfile, Tier, WeightShard,
};
use agent_core::system_prompt::{
    generation_phrases_are_all_refusals, DECLARED_LIMITS, AGENT_SYSTEM_PROMPT,
};
...
pub enum ServeEngine {
    ...
    /// principle of `src/agent/effort.rs`: "A Agent operator answers with the
...
///
/// Agent consensus requires bit-identical equality (the `output_commitment` grouping),
...
pub struct ServeConfig {
    ...
    /// The name served through the API - tier naming: `agent-{tier}-{version}`.
...
/// (we do not take a third-party model and sell it as Agent - only our own layer
/// carries the Agent name; the base is stated openly in `NOTICE.md` and the model card).
///
/// # Errors
///
...
/// [`AGENT_SYSTEM_PROMPT`] so that every bridge serves the same text, and the
/// `agent-prompt-is-true` gate has a single place to check against the chain
...
/// edit:
///
/// * every generation phrase in it is a refusal, not an offer. Agent reads and
...
///
/// These mirror the `REQUIRED_CLAIMS` of the `agent-prompt-is-true` gate. The
...
///
/// These mirror the `EFFORT_FIGURES` of the `agent-prompt-is-true` gate. The
/// gate derives the figures from the on-chain constants in `src/agent/effort.rs`
...
pub fn checked_system_prompt() -> Result<&'static str, String> {
    verify_prompt_text(AGENT_SYSTEM_PROMPT)?;
    Ok(AGENT_SYSTEM_PROMPT)
}
...
/// Multiplier tag pattern such as `0.5x`, `2x`, `10x`. Agent tiers
...
mod tests {
    ...
    fn default_config_is_light_tier() {
        ...
        assert_eq!(cfg.served_model_name, "agent-light-v0.1");
    ...
    fn normal_tier_maps_to_pro_weights_but_our_name() {
        ...
        assert_eq!(cfg.served_model_name, "agent-normal-v0.1");
        assert!(assert_served_name_is_ours(&cfg).is_ok());
    }
    ...
    fn third_party_name_in_served_alias_is_rejected() {
        let cfg = ServeConfig {
            served_model_name: "agent-deepseek-v1".to_string(),
            ..Default::default()
        };
    ...
    fn multiplier_labels_are_rejected() {
        for bad in ["agent-0.5x", "agent-10x-v1", "agent-2x"] {
            let cfg = ServeConfig {
    ...
    /// asserted `std::ptr::eq` and failed: `AGENT_SYSTEM_PROMPT` is a `const`,
    ...
    fn each_declared_limit_is_stated_in_the_prompt() {
        for (medium, unit, limit) in DECLARED_LIMITS {
            assert!(
                AGENT_SYSTEM_PROMPT.contains(&limit.to_string()),
    ...
    fn a_prompt_missing_a_closed_loop_channel_is_refused() {
        let stripped = AGENT_SYSTEM_PROMPT.replace("SocialFi reference", "<missing>");
    ...
    fn a_prompt_missing_an_effort_figure_is_refused() {
        let stripped = AGENT_SYSTEM_PROMPT.replace("10.0x", "<missing>");
    ...
    fn a_prompt_missing_the_tier_naming_is_refused() {
        let stripped =
            AGENT_SYSTEM_PROMPT.replace("Tiers are called light and normal.", "<missing>");
    ...
    fn the_shipped_prompt_passes_the_check() {
        assert!(verify_prompt_text(AGENT_SYSTEM_PROMPT).is_ok());
    }
