...
pub enum Command {
    ...
    /// (train/eval identity, `agent-tune::eval`); an empty corpus or a
    /// dataset that names no model is refused.
    Tune { dataset: Option<String> },
    /// Health summary.
    Status,
    /// Run a JSONL data file through the schema gate (agent-tune::schema).
    Validate { path: Option<String> },
    /// Grade a golden dataset against produced responses and report the score,
    /// the gate verdict and the dataset digest (agent-tune::eval).
    ///
    ...
    /// (`agent_serve::config::checked_system_prompt`) and prints the verified
...
/// The help text.
pub const HELP: &str = "\
agent-ops - the Agent off-chain operator CLI (skeleton)

Usage:
  agent-ops register [MODEL_ID_HEX]   model registration draft
  agent-ops bond [AMOUNT]             operator compute-bond draft
  agent-ops serve                     serving-bridge summary
  agent-ops tune [DATASET]            training plan draft (with a dataset:
                                      attach its eval-set digest to the plan)
  agent-ops status                    health summary
  agent-ops validate [JSONL_FILE]     data set schema gate (empty field, byte
                                      ceiling, line-numbered error, TR ratio)
  agent-ops eval [DATASET] [RESPONSES] [MIN_SCORE]
                                      grade a golden set against produced
                                      responses (score, gate verdict, digest;
                                      a non-clearing report exits non-zero)
  agent-ops prompt                    print the verified serving system prompt
  agent-ops help                      this text
