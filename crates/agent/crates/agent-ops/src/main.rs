// Unsafe lock: the whole agent workspace is at 0 unsafe. This binary joins
...
#![forbid(unsafe_code)]
//! The agent-ops entry point. A skeleton: it parses commands and prints
//! drafts.
...
use agent_core::dataset::DatasetError;
use agent_core::model::{FineTuneSource, ModelId, ModelLicense, ModelSpec};
use agent_core::tier::ModelTier;
use agent_serve::config::ServeConfig;
use agent_tune::eval::{
    run_eval, CaseReport, CheckOutcome, EvalCase, EvalDataSet, EvalGate, EvalReport,
};
use agent_tune::plan::TunePlan;
...
fn load_records(path: &str) -> Vec<agent_data::jsonl::InstructionRecord> {
    ...
    for line in text.lines() {
        match agent_data::jsonl::decode(line) {
            Ok(r) => records.push(r),
            Err(e) => {
...
fn main() {
    ...
    match parse(&argv) {
        Command::Register { model_id_hex } => {
            println!("Agent model registration (a draft)");
            println!(
                "model_id: {}",
        ...
        Command::Serve => {
            ...
            println!(
                ...
                agent_integrations::rest::MAX_BACKOFF_SECS
            );
        }
        Command::Tune { dataset } => {
            ...
            if let Some(dp) = dataset {
                ...
                // label gate is `agent-core`'s, run here before the digest.
                let meta = agent_core::dataset::DatasetMetadata::training(
                    plan.base.0,
                    records.len() as u64,
                );
                match meta.validate() {
        ...
        Command::Status => {
            println!(
                "agent-ops status: a skeleton - the chain connection is fail-closed (NotConnected)"
            );
        }
        Command::Eval {
            dataset,
        ...
        Command::Validate { path } => match path {
            ...
            Some(p) => match std::fs::read_to_string(&p) {
                ...
                Ok(text) => {
                    ...
                    match agent_tune::schema::validate_records(&lines) {
                        Err(e) => {
                            eprintln!("THE SCHEMA GATE REFUSED: {e:?}");
                            std::process::exit(1);
                        }
                        Ok(records) => {
                            let ratio = agent_tune::schema::tr_ratio_estimate(&records);
                            println!(
