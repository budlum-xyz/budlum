//! The Agent system prompt, as a compiled-in constant.
//!
...
//! protocol by tests below and by the `agent-prompt-is-true` repository gate.
//!
...
//! constants in `src/agent/perception.rs` and fail when the two drift. A
...
//! # Language
//!
//! English, like the rest of the tree. Agent answering a Turkish question in
...
/// The system prompt handed to a Agent model before the first user message.
///
...
pub const AGENT_SYSTEM_PROMPT: &str = r#"You are Agent, the reading layer of Budlum.
...
Agent is the layer that reads what B.U.D. stores, under the permissions Pollen
...
Verification of a Agent answer is three separate bindings, and each closes a
different lie.
...
upstream engines are named honestly wherever they are used, and the name Agent
...
/// below and the `agent-prompt-is-true` gate can both check that the numbers
/// in the prompt are the numbers the chain enforces. The chain-side constants
/// live in `src/agent/perception.rs`, in a crate this workspace deliberately
...
/// `agent-prompt-is-true` gate - and the gate reads the source text of this
...
mod tests {
    ...
    fn every_declared_limit_appears_in_the_prompt_as_an_exact_integer() {
        for (modality, unit, value) in DECLARED_LIMITS {
            ...
            assert!(
                AGENT_SYSTEM_PROMPT.contains(&line),
    ...
    fn the_prompt_offers_no_generation_surface() {
        if let Err(finding) = generation_phrases_are_all_refusals(AGENT_SYSTEM_PROMPT) {
            panic!("{finding}");
        }
    }
    ...
    fn the_reading_only_boundary_carries_its_three_reasons() {
        for anchor in [
            ...
            assert!(
                AGENT_SYSTEM_PROMPT.contains(anchor),
    ...
    fn the_prompt_does_not_overstate_what_is_proven() {
        let lower = AGENT_SYSTEM_PROMPT.to_lowercase();
        for overclaim in [
        ...
        assert!(
            AGENT_SYSTEM_PROMPT.contains("is not something the field can do yet"),
    ...
    fn tier_names_are_ours() {
        assert!(AGENT_SYSTEM_PROMPT.contains("Tiers are called light and normal."));
        assert!(!AGENT_SYSTEM_PROMPT.to_lowercase().contains("deepseek"));
    }
    ...
    fn the_closed_loop_channels_are_named() {
        for channel in ["Pollen grant", "B.U.D. storage deal", "SocialFi reference"] {
            assert!(
                AGENT_SYSTEM_PROMPT.contains(channel),
    ...
    fn the_prompt_explains_the_four_layers() {
        for layer in ["B.U.D.", "BudZero", "Pollen", "consensus domains"] {
            assert!(
                AGENT_SYSTEM_PROMPT.contains(layer),
