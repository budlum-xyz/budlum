...
pub struct ModelSpec {
    ...
    /// (see agent-data::verify).
    pub sha256: Option<String>,
    pub license: ModelLicense,
    pub fine_tune_source: FineTuneSource,
    /// The Agent tier this checkpoint supports (`light` / `normal`).
    pub tier: ModelTier,
}
...
mod tests {
    ...
    fn placeholder_digest_is_deterministic() {
        assert_eq!(placeholder_digest(b"agent"), placeholder_digest(b"agent"));
        assert_ne!(placeholder_digest(b"agent"), placeholder_digest(b"lobot"));
    }
}
