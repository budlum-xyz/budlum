...
mod tests {
    ...
    fn correct_hash_passes() {
        let hex = hex_of(&content_id_of(b"agent"));
        assert_eq!(verify_sha256(b"agent", &hex), Ok(()));
    }
    ...
    fn content_id_is_deterministic_and_32_bytes() {
        assert_eq!(content_id_of(b"agent"), content_id_of(b"agent"));
        assert_eq!(content_id_of(b"agent").len(), 32);
    }
}
