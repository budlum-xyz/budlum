...
use lubot_core::dataset::DatasetError;
...
fn load_records(path: &str) -> Vec<lubot_data::jsonl::InstructionRecord> {
    ...
    match parse(&argv) {
        ...
        Command::Serve => {
            ...
            println!(
                ...
                lubot_integrations::rest::MAX_BACKOFF_SECS
            );
        }
        Command::Tune { dataset } => {
            ...
            if let Some(dp) = dataset {
                ...
                // label gate is `lubot-core`'s, run here before the digest.
                let meta = lubot_core::dataset::DatasetMetadata::training(
                    plan.base.0,
                    records.len() as u64,
                );
                match meta.validate() {
