...
/// Inference engine (research section 1.4: a resident-batch engine and a resident-graph engine are day-zero supported).
...
pub enum ServeEngine {
    ...
    ///
    /// a resident-batch engine and a resident-graph engine require the whole model to be resident in VRAM, which
    ...
    /// operator too.
    ///
    /// It is spoken to as a separate process over an a model vendor-compatible endpoint; no code
...
impl ServeEngine {
    ...
    pub const fn is_bitwise_reproducible(self) -> bool {
        match self {
            ...
            ServeEngine::ResidentBatch | ServeEngine::ResidentGraph | ServeEngine::QuantizedLocal => true,
...
mod tests {
    ...
    fn third_party_name_in_served_alias_is_rejected() {
        let cfg = ServeConfig {
            served_model_name: "lubot-upstream-v1".to_string(),
            ..Default::default()
        };
