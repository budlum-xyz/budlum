...
- **Taban model:** the upstream vendor V4 ailesi (MIT ağırlıklar; `V4-Flash-Base` varsayılan, karar ilk ince ayar öncesi yeniden onaylanır).
- **Kademe adlandırması (2026-08-13):** the upstream vendor'in varyant adları Lubot'ta kullanılmaz. Flash tabanlı kademe **`lubot-light`**, Pro tabanlı kademe **`lubot-normal`** olarak sunulur. Çarpan/kat etiketleri (0.5x, 10x vb.) Lubot'ta yoktur - denetim kodda (`lubot-serve::config::assert_served_name_is_ours`).
- **Atıf politikası:** "the upstream vendor → Lubot" isim değişikliği yalnızca kendi kodumuzda yapılır. Kopyalanan üçüncü taraf kodu ve ağırlık adları olduğu gibi kalır; MIT bildirimi ve "tabanıdır" atfı `NOTICE.md` ve model kartında yer alır.
...
| `lubot-serve` | a resident-batch engine/a resident-graph engine köprüsü yapılandırması (ağırlık adı korunur, sunulan ad bizimdir) |
...
- `docs/ARASTIRMA_RAPORU_2026-08-13.md` - the upstream vendor V4 araştırması
