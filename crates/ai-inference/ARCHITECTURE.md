...
This file summarises the layers.
...
[ This repo - the off-chain skeleton ]
   ai-core   : ModelId (a 32-byte hash, mirroring the AiModelId form),
                  dataset types, LoRaManifest
   ai-data   : closed-loop source checking over 3 channels, the record
                  format, fail-closed hash verification
   ai-serve  : the serving bridge; the weight name is preserved and
                  the served name is ours
   ai-tune   : TunePlan (LoRA in BF16 or FP16, with no FP4), the output hash
                  lock
   ai-ops    : the CLI skeleton
```
...
1. **Closed loop:** there is no path that reads outside data; every set enters
   ...
   verification.
2. **Fail closed:** an unverified hash is a refusal, and `ai-data::verify`
...
| K7 tier naming | **light** (the smaller base) and **normal** (the larger base); there are no multiplier labels |

## Deepening

- `ai-data`: real SHA-256 verification (`verify_sha256`, `content_id_of`),
  JSONL records built on serde_json, and a draft structural chat template.
- `ai-serve`: the tier naming (`ai_inference-light-*` and `ai_inference-normal-*`), the
  multiplier label check, and a fail-closed chain RPC draft
  (`chain::NotConnected`).
- `ai-core`: the `ModelTier` type, light or normal; `ModelSpec` carries the
  tier.

## The system prompt

The prompt handed to a model before the first user message lives in
`ai-core/src/system_prompt.rs` as a `const`, not in a configuration file.
...
- `ai-core` tests assert the prompt is internally consistent - the ceilings
...
The prompt states why AI inference layer reads and does not generate, at length and with the
...
`ai-serve/src/residency.rs` plans where each part of a model lives on the
