# AI inference layer architecture summary
...
```
[ On chain - budlum/main/src/ai_inference/ ]        (exists; this repo does not touch it)
   model registry - operator compute bond - Pollen - B.U.D. - SocialFi
                        ^ hash and reference matching
[ This repo - the off-chain skeleton ]
   ai-inference-core   : ModelId (a 32-byte hash, mirroring the AiModelId form),
                  dataset types, LoRaManifest
   ai-inference-data   : closed-loop source checking over 3 channels, the record
                  format, fail-closed hash verification
   ai-inference-serve  : the vLLM and SGLang bridge; the weight name is preserved and
                  the served name is ours
   ai-inference-tune   : TunePlan (LoRA in BF16 or FP16, with no FP4), the output hash
                  lock
   ai-inference-ops    : the CLI skeleton
```
...
1. **Closed loop:** there is no path that reads outside data; every set enters
   ...
   verification.
2. **Fail closed:** an unverified hash is a refusal, and `ai-inference-data::verify`
...
3. **Attribution:** third party names are preserved; only our own layer carries
   the name "the AI inference layer".
...
- `ai-inference-data`: real SHA-256 verification (`verify_sha256`, `content_id_of`),
  JSONL records built on serde_json, and a draft structural chat template.
- `ai-inference-serve`: the tier naming (`ai-inference-light-*` and `ai-inference-normal-*`), the
  multiplier label check, and a fail-closed chain RPC draft
  (`chain::NotConnected`).
- `ai-inference-core`: the `ModelTier` type, light or normal; `ModelSpec` carries the
  tier.

## The system prompt, 2026-08-25

The prompt handed to a model before the first user message lives in
`ai-inference-core/src/system_prompt.rs` as a `const`, not in a configuration file.
...
- `ai-inference-core` tests assert the prompt is internally consistent - the ceilings
  ...
  proven.
- The `ai-inference-prompt-is-true` gate checks the half those tests cannot see: that
  the numbers the prompt tells a user are the numbers `src/ai_inference/perception.rs`
  actually admits by. It fails in both directions, because which of the two is
  wrong is a judgement a gate should not make.

The prompt states why the AI inference layer reads and does not generate, at length and with the
...
`ai-inference-prompt-is-true` therefore carries a table of (sentence, file, symbol)
...
`ai-inference-serve/src/residency.rs` plans where each part of a model lives on the
