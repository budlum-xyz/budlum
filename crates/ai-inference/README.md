...
> AI inference layer work is collected into a single pull request.
...
- **Tier naming, 2026-08-13:** DeepSeek's variant names are not used in the AI inference layer.
  The Flash-based tier is served as **`ai_inference-light`** and the Pro-based tier as
  **`ai_inference-normal`**. Multiplier labels such as 0.5x or 10x do not exist in
...
| `ai-serve` | The vLLM and SGLang bridge configuration; the weight name is preserved and the served name is ours |
