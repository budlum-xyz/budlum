...
mod tests {
    ...
    fn a_bridge_that_passes_every_check_starts() {
        ...
        assert!(bridge.summary().contains("ai-inference-light"));
    }
    ...
    fn a_third_party_name_is_refused_before_the_device_is_measured() {
        let mut c = cfg();
        c.served_model_name = String::from("deepseek-v4-flash");
        let err = Bridge::start(
            c,
            &[9; 32],
            &shards(),
    ...
    fn a_started_bridge_serves_the_checked_prompt() {
        ...
        assert!(bridge.system_prompt().contains("the AI inference layer"));
    }
