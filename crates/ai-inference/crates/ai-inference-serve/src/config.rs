...
use ai_inference_core::tier::ModelTier;

use crate::residency::{
    DeviceBudget, Placement, PlanError, ResidencyPlan, SemanticProfile, Tier, WeightShard,
};
use ai_inference_core::system_prompt::{
    generation_phrases_are_all_refusals, AI_INFERENCE_SYSTEM_PROMPT, DECLARED_LIMITS,
};
...
pub enum ServeEngine {
    ...
    /// principle of `src/ai_inference/effort.rs`: "A the AI inference layer operator answers with the
...
/// (we do not take a third-party model and sell it as the AI inference layer - only our own layer
/// carries the the AI inference layer name; the base is stated openly in `NOTICE.md` and the model card).
///
/// # Errors
///
...
/// [`AI_INFERENCE_SYSTEM_PROMPT`] so that every bridge serves the same text, and the
...
/// edit:
///
/// * every generation phrase in it is a refusal, not an offer. the AI inference layer reads and
...
pub fn checked_system_prompt() -> Result<&'static str, String> {
    verify_prompt_text(AI_INFERENCE_SYSTEM_PROMPT)?;
    Ok(AI_INFERENCE_SYSTEM_PROMPT)
}
...
/// Multiplier tag pattern such as `0.5x`, `2x`, `10x`. the AI inference layer tiers
...
mod tests {
    ...
    fn default_config_is_light_tier() {
        ...
        assert_eq!(cfg.served_model_name, "ai-inference-light-v0.1");
    ...
    fn normal_tier_maps_to_pro_weights_but_our_name() {
        ...
        assert_eq!(cfg.served_model_name, "ai-inference-normal-v0.1");
        assert!(assert_served_name_is_ours(&cfg).is_ok());
    }
    ...
    fn third_party_name_in_served_alias_is_rejected() {
        let cfg = ServeConfig {
            served_model_name: "ai_inference-deepseek-v1".to_string(),
            ..Default::default()
        };
    ...
    /// asserted `std::ptr::eq` and failed: `AI_INFERENCE_SYSTEM_PROMPT` is a `const`,
    ...
    fn the_bridge_serves_the_checked_prompt() {
        ...
        assert_eq!(
            prompt, AI_INFERENCE_SYSTEM_PROMPT,
    ...
    fn each_declared_limit_is_stated_in_the_prompt() {
        for (medium, unit, limit) in DECLARED_LIMITS {
            assert!(
                AI_INFERENCE_SYSTEM_PROMPT.contains(&limit.to_string()),
    ...
    fn a_prompt_missing_a_closed_loop_channel_is_refused() {
        let stripped = AI_INFERENCE_SYSTEM_PROMPT.replace("SocialFi reference", "<missing>");
    ...
    fn a_prompt_missing_an_effort_figure_is_refused() {
        let stripped = AI_INFERENCE_SYSTEM_PROMPT.replace("10.0x", "<missing>");
    ...
    fn a_prompt_missing_the_tier_naming_is_refused() {
        let stripped =
            AI_INFERENCE_SYSTEM_PROMPT.replace("Tiers are called light and normal.", "<missing>");
    ...
    fn the_shipped_prompt_passes_the_check() {
        assert!(verify_prompt_text(AI_INFERENCE_SYSTEM_PROMPT).is_ok());
    }
