...
//! * **the AI inference layer serving** - token-metered like an API: every prompt debits its
...
#[must_use]
pub fn bud_extension_fee_usd_per_tb(
    ten_year_storage_cost_usd: f64,
    custody_years: u32,
) -> Option<f64> {
...
/// A the AI inference layer wallet, debited token by token as prompts run.
...
mod fee_scenario_tests {
    ...
    fn custody_extension_is_pro_rata_and_capped_at_a_hundred_years() {
        ...
        assert!(approx(
            bud_extension_fee_usd_per_tb(ten_year, 10).unwrap(),
            0.0
        ));
        // Each year above ten is one tenth of the ten-year cost.
        assert!(approx(
            bud_extension_fee_usd_per_tb(ten_year, 20).unwrap(),
            ten_year
        ));
        assert!(approx(
            bud_extension_fee_usd_per_tb(ten_year, 55).unwrap(),
            ten_year * 4.5
        ));
        // The ceiling is a hundred years: nine times the ten-year cost extra.
        assert!(approx(
            bud_extension_fee_usd_per_tb(ten_year, 100).unwrap(),
            ten_year * 9.0
        ));
    ...
    #[test]
    fn ai_inference_charge_debits_tokens_times_rate() {
