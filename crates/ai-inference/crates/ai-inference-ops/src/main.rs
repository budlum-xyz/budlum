...
#![forbid(unsafe_code)]
//! The ai-inference-ops entry point. A skeleton: it parses commands and prints
//! drafts.

mod cli;
mod logparse;

use ai_inference_core::dataset::DatasetError;
use ai_inference_core::model::{FineTuneSource, ModelId, ModelLicense, ModelSpec};
use ai_inference_core::tier::ModelTier;
use ai_inference_serve::config::ServeConfig;
use ai_inference_tune::eval::{
    run_eval, CaseReport, CheckOutcome, EvalCase, EvalDataSet, EvalGate, EvalReport,
};
use ai_inference_tune::plan::TunePlan;
...
fn load_records(path: &str) -> Vec<ai_inference_data::jsonl::InstructionRecord> {
    ...
    for line in text.lines() {
        match ai_inference_data::jsonl::decode(line) {
            Ok(r) => records.push(r),
            Err(e) => {
...
fn main() {
    ...
    match parse(&argv) {
        Command::Register { model_id_hex } => {
            println!("the AI inference layer model registration (a draft)");
            println!(
                "model_id: {}",
        ...
        Command::Serve => {
            ...
            println!(
                ...
                ai_inference_integrations::rest::MAX_BACKOFF_SECS
            );
        }
        Command::Tune { dataset } => {
            ...
            if let Some(dp) = dataset {
                ...
                // label gate is `ai-inference-core`'s, run here before the digest.
                let meta = ai_inference_core::dataset::DatasetMetadata::training(
                    plan.base.0,
                    records.len() as u64,
                );
                match meta.validate() {
        ...
        Command::Status => {
            println!(
                "ai-inference-ops status: a skeleton - the chain connection is fail-closed (NotConnected)"
            );
        }
        Command::Eval {
            dataset,
        ...
        Command::Prompt => match ai_inference_serve::config::checked_system_prompt() {
        ...
        Command::Validate { path } => match path {
            ...
            Some(p) => match std::fs::read_to_string(&p) {
                ...
                Ok(text) => {
                    ...
                    match ai_inference_tune::schema::validate_records(&lines) {
                        Err(e) => {
                            eprintln!("THE SCHEMA GATE REFUSED: {e:?}");
                            std::process::exit(1);
                        }
                        Ok(records) => {
                            let ratio = ai_inference_tune::schema::tr_ratio_estimate(&records);
                            println!(
