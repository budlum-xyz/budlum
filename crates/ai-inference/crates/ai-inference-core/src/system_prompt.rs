//! The the AI inference layer system prompt, as a compiled-in constant.
//!
...
//! # Language
//!
//! English, like the rest of the tree. the AI inference layer answering a Turkish question in
...
/// The system prompt handed to a the AI inference layer model before the first user message.
///
...
pub const AI_INFERENCE_SYSTEM_PROMPT: &str = r#"You are the AI inference layer, the reading layer of Budlum.
...
AI inference layer is the layer that reads what B.U.D. stores, under the permissions Pollen
...
Verification of a the AI inference layer answer is three separate bindings, and each closes a
different lie.
...
upstream engines are named honestly wherever they are used, and the name the AI inference layer
...
mod tests {
    ...
    fn every_declared_limit_appears_in_the_prompt_as_an_exact_integer() {
        for (modality, unit, value) in DECLARED_LIMITS {
            ...
            assert!(
                AI_INFERENCE_SYSTEM_PROMPT.contains(&line),
    ...
    fn the_prompt_offers_no_generation_surface() {
        if let Err(finding) = generation_phrases_are_all_refusals(AI_INFERENCE_SYSTEM_PROMPT) {
            panic!("{finding}");
        }
    }
    ...
    fn the_reading_only_boundary_carries_its_three_reasons() {
        for anchor in [
            ...
            assert!(
                AI_INFERENCE_SYSTEM_PROMPT.contains(anchor),
    ...
    fn the_prompt_does_not_overstate_what_is_proven() {
        let lower = AI_INFERENCE_SYSTEM_PROMPT.to_lowercase();
        for overclaim in [
        ...
        assert!(
            AI_INFERENCE_SYSTEM_PROMPT.contains("is not something the field can do yet"),
    ...
    fn tier_names_are_ours() {
        assert!(AI_INFERENCE_SYSTEM_PROMPT.contains("Tiers are called light and normal."));
        assert!(!AI_INFERENCE_SYSTEM_PROMPT
            .to_lowercase()
            .contains("deepseek"));
    }
    ...
    fn the_closed_loop_channels_are_named() {
        for channel in ["Pollen grant", "B.U.D. storage deal", "SocialFi reference"] {
            assert!(
                AI_INFERENCE_SYSTEM_PROMPT.contains(channel),
    ...
    fn the_prompt_explains_the_four_layers() {
        for layer in ["B.U.D.", "BudZero", "Pollen", "consensus domains"] {
            assert!(
                AI_INFERENCE_SYSTEM_PROMPT.contains(layer),
