...
pub enum Command {
    ...
    /// (train/eval identity, `ai-tune::eval`); an empty corpus or a
    /// dataset that names no model is refused.
    Tune { dataset: Option<String> },
    /// Health summary.
    Status,
    /// Run a JSONL data file through the schema gate (ai-tune::schema).
    Validate { path: Option<String> },
    /// Grade a golden dataset against produced responses and report the score,
    /// the gate verdict and the dataset digest (ai-tune::eval).
    ///
    ...
    /// (`ai_serve::config::checked_system_prompt`) and prints the verified
...
/// The help text.
pub const HELP: &str = "\
ai-ops - the AI inference layer off-chain operator CLI (skeleton)

Usage:
  ai-ops register [MODEL_ID_HEX]   model registration draft
  ai-ops bond [AMOUNT]             operator compute-bond draft
  ai-ops serve                     serving-bridge summary
  ai-ops tune [DATASET]            training plan draft (with a dataset:
                                      attach its eval-set digest to the plan)
  ai-ops status                    health summary
  ai-ops validate [JSONL_FILE]     data set schema gate (empty field, byte
                                      ceiling, line-numbered error, TR ratio)
  ai-ops eval [DATASET] [RESPONSES] [MIN_SCORE]
                                      grade a golden set against produced
                                      responses (score, gate verdict, digest;
                                      a non-clearing report exits non-zero)
  ai-ops prompt                    print the verified serving system prompt
  ai-ops help                      this text
