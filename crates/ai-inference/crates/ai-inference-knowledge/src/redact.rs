...
//!
//! Closed-circuit principle: no text entering the AI inference layer may contain an API key,
...
fn is_anthropic_key(v: &str) -> bool {
    // Anthropic keys are `sk-ant-api03-...`, long and prefixed.
    v.len() >= 20
...
fn is_openai_key(v: &str) -> bool {
    // OpenAI project/scoped keys are `sk-proj-...` (long). The generic `sk-`
    // prefix is already covered by `is_deepseek_key`; this catches the two
    // well-known scoped families and OpenAI legacy `sk-`.
    v.len() >= 20
...
const VALUE_PATTERNS: &[(&str, ValuePredicate)] = &[
    ("anthropic_key", is_anthropic_key),
    ("openai_key", is_openai_key),
...
mod tests {
    ...
    /// An Anthropic key in free text is masked.
    #[test]
    fn anthropic_key_in_free_text_is_redacted() {
    ...
    /// An OpenAI scoped key in free text is masked.
    #[test]
    fn openai_scoped_key_is_redacted() {
