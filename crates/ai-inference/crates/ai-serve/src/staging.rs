...
pub fn stripe_plan(len: u64, devices: &[StorageDevice]) -> Result<Vec<StripeChunk>, StripeError> {
    ...
    for dev in devices {
        let share = (u128::from(len) * u128::from(dev.bandwidth_kib_s)) / u128::from(total_weight);
...
mod staging_tests {
    ...
    fn a_slow_leg_gets_a_proportionally_small_share() {
        ...
        assert!(
            slow.len < equal_third / 2,
            "slow leg {} must be under half an equal third {}",
            slow.len,
            equal_third / 2
        );
        assert!(
            fast.len > equal_third * 13 / 10,
            "fast leg {} must exceed 1.3x an equal third",
            fast.len
        );
    ...
    fn chunks_sum_exactly_and_are_contiguous_for_every_rotation() {
        ...
        for rotation in 0..devices.len() {
            ...
            for (i, c) in plan.iter().enumerate() {
                ...
                assert!(
                    c.len <= len,
                    "rotation {rotation} chunk {i} longer than the read"
                );
                cursor += c.len;
            }
    ...
    fn zero_length_no_device_are_refused() {
        assert_eq!(
            stripe_plan(0, &[dev(1, 100)]).unwrap_err(),
            StripeError::NothingToStripe
        );
