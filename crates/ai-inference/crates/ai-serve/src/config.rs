...
use ai_core::system_prompt::{
    generation_phrases_are_all_refusals, AI_SYSTEM_PROMPT, DECLARED_LIMITS,
};

/// Inference engine, named by the shape of the engine rather than by product.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ServeEngine {
    /// Whole model resident in VRAM, continuous batching.
    ResidentBatch,
    /// Whole model resident in VRAM, compiled execution graph.
    ResidentGraph,
    /// Quantized weights on consumer hardware, CPU or a small GPU.
    QuantizedLocal,
    ...
    /// principle of `src/ai_inference/effort.rs`: "An AI inference layer operator answers with the
...
impl ServeEngine {
    ...
    pub const fn is_bitwise_reproducible(self) -> bool {
        match self {
            ...
            ServeEngine::ResidentBatch
            | ServeEngine::ResidentGraph
            | ServeEngine::QuantizedLocal => true,
...
///
/// AI inference layer consensus requires bit-identical equality (the `output_commitment` grouping),
...
pub struct ServeConfig {
    ...
    /// The name served through the API - tier naming: `ai_inference-{tier}-{version}`.
...
impl ServeConfig {
    /// Build a configuration from tier + version.
    #[must_use]
...
/// (we do not take a third-party model and sell it as AI inference layer - only our own layer
/// carries the AI inference layer name; the base is stated openly in the
/// attribution notice and the model card).
///
/// # Errors
///
...
pub fn assert_served_name_is_ours(cfg: &ServeConfig) -> Result<(), String> {
    ...
    if name.contains("example-org") || name.contains("upstream-base") {
        return Err(format!(
...
/// [`AI_SYSTEM_PROMPT`] so that every bridge serves the same text, and the
/// `ai-inference-prompt-is-true` gate has a single place to check against the chain
...
/// edit:
///
/// * every generation phrase in it is a refusal, not an offer. AI inference layer reads and
...
///
/// These mirror the `REQUIRED_CLAIMS` of the `ai-inference-prompt-is-true` gate. The
...
///
/// These mirror the `EFFORT_FIGURES` of the `ai-inference-prompt-is-true` gate. The
/// gate derives the figures from the on-chain constants in `src/ai_inference/effort.rs`
...
pub fn checked_system_prompt() -> Result<&'static str, String> {
    verify_prompt_text(AI_SYSTEM_PROMPT)?;
    Ok(AI_SYSTEM_PROMPT)
}
...
/// Multiplier tag pattern such as `0.5x`, `2x`, `10x`. AI inference layer tiers
...
mod tests {
    ...
    fn default_config_is_light_tier() {
        ...
        assert_eq!(cfg.served_model_name, "ai_inference-light-v0.1");
    ...
    fn normal_tier_maps_to_pro_weights_but_our_name() {
        ...
        assert_eq!(cfg.served_model_name, "ai_inference-normal-v0.1");
        assert!(assert_served_name_is_ours(&cfg).is_ok());
    }
    ...
    fn third_party_name_in_served_alias_is_rejected() {
        let cfg = ServeConfig {
            served_model_name: "ai_inference-upstream-base-v1".to_string(),
            ..Default::default()
        };
    ...
    /// asserted `std::ptr::eq` and failed: `AI_SYSTEM_PROMPT` is a `const`,
    ...
    fn each_declared_limit_is_stated_in_the_prompt() {
        for (medium, unit, limit) in DECLARED_LIMITS {
            assert!(
                AI_SYSTEM_PROMPT.contains(&limit.to_string()),
    ...
    fn a_prompt_missing_a_closed_loop_channel_is_refused() {
        let stripped = AI_SYSTEM_PROMPT.replace("SocialFi reference", "<missing>");
    ...
    fn a_prompt_missing_an_effort_figure_is_refused() {
        let stripped = AI_SYSTEM_PROMPT.replace("10.0x", "<missing>");
    ...
    fn a_prompt_missing_the_tier_naming_is_refused() {
        let stripped = AI_SYSTEM_PROMPT.replace("Tiers are called light and normal.", "<missing>");
    ...
    fn the_shipped_prompt_passes_the_check() {
        assert!(verify_prompt_text(AI_SYSTEM_PROMPT).is_ok());
    }
