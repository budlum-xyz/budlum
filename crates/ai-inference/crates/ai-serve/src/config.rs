...
use ai_core::system_prompt::{
    generation_phrases_are_all_refusals, DECLARED_LIMITS, AI_SYSTEM_PROMPT,
};
...
/// carries the AI inference layer name; the base is stated openly in `NOTICE.md` and the model card).
///
/// # Errors
///
...
mod tests {
    ...
    fn multiplier_labels_are_rejected() {
        for bad in ["ai_inference-0.5x", "ai_inference-10x-v1", "ai_inference-2x"] {
            let cfg = ServeConfig {
    ...
    fn a_prompt_missing_the_tier_naming_is_refused() {
        let stripped =
            AI_SYSTEM_PROMPT.replace("Tiers are called light and normal.", "<missing>");
