...
//!
//! The on-chain `register_ai_inference_model(model_hash)` record has to match this
//! digest: an output not derived from the same digest is not accepted.

use ai_inference_core::manifest::LoRaManifest;
use ai_inference_core::model::{Hash32, ModelId};
