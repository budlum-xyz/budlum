//! Behavior evaluation: a model-agnostic correctness harness for the AI inference layer.
//!
//! The "success" of the the AI inference layer module must be measurable
...
impl EvalDataSet {
    ...
    pub fn from_golden(
        name: impl Into<String>,
        records: &[ai_inference_data::jsonl::InstructionRecord],
    ) -> Self {
        let cases = records
            .iter()
            .map(|r| EvalCase {
...
/// A success threshold. "the AI inference layer is usable and successful" is not a feeling - it
...
mod tests {
    ...
    fn contains_all_requires_every_needle() {
        let c = EvalCase {
            name: "parts".into(),
            prompt: "Which layers is the AI inference layer built from?".into(),
    ...
    fn curriculum_dataset_has_a_stable_digest_and_is_graded_end_to_end() {
        ...
        const CURRICULUM: &str = include_str!("../data/ai_inference-davranis-curriculum.jsonl");
        let mut records = Vec::new();
        for line in CURRICULUM.lines() {
            records.push(ai_inference_data::jsonl::decode(line).expect("valid JSONL line"));
        }

        let set = EvalDataSet::from_golden("ai_inference-davranis-curriculum", &records);
        ...
        assert_eq!(
            d1,
            EvalDataSet::from_golden("ai_inference-davranis-curriculum", &records).digest()
        );
        assert_eq!(d1.len(), 32);
    ...
    fn from_golden_maps_user_become_prompt_and_assistant_becomes_rule() {
        let recs = vec![ai_inference_data::jsonl::InstructionRecord {
            system: None,
    ...
    fn eval_set_binds_to_the_plan_it_is_registered_in() {
        ...
        let mut plan =
            crate::plan::TunePlan::lora(ai_inference_core::model::ModelId([2; 32]), 2_000);
        ...
        let other = EvalDataSet::from_golden(
            "t",
            &[ai_inference_data::jsonl::InstructionRecord {
                system: None,
                user: "question".to_string(),
