    ...
    pub fn handle_response(
        ...
        if response.cid != requested_cid {
            tracing::warn!(
                ...
                got = %response.cid,
                "received response for an unrequested cid - rejecting"
            );
            return Err(StoreError::UnexpectedCid {
                wanted: requested_cid,
                got: response.cid,
            });
        }
...
mod tests {
    ...
    fn handle_response_stores_valid_chunk() {
        ...
        bitswap.handle_response(cid, resp).unwrap();
    ...
    fn handle_response_rejects_integrity_mismatch() {
        ...
        let result = bitswap.handle_response(wrong_cid, resp);
        assert!(matches!(result, Err(StoreError::IntegrityMismatch { .. })));
        assert!(!store.has(&wrong_cid));
    }

    #[test]
    fn handle_response_rejects_unrequested_cid() {
        let store = Arc::new(MemoryContentStore::with_default_capacity());
        let bitswap = BudBitswap::new(store.clone());

        let wanted = ContentId::of(b"wanted block");
        let data = b"attacker chosen block".to_vec();
        let got = ContentId::of(&data);
        let resp = BitswapResponse {
            cid: got,
            data,
            not_found: false,
        ...
        let result = bitswap.handle_response(wanted, resp);
        assert!(matches!(result, Err(StoreError::UnexpectedCid { .. })));
        assert!(!store.has(&got), "istenmeyen blok saklanmamali");
    }
