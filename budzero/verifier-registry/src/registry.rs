...
impl VerifierRegistry {
    ...
    pub fn register_agent_operator(
        ...
        self.register(
            account,
            crate::role::roles::AGENT_OPERATOR,
            stake,
            current_epoch,
        )
    }
    ...
    pub fn is_active_agent_operator(&self, account: &Address) -> bool {
        self.get(account, crate::role::roles::AGENT_OPERATOR)
            .is_some_and(Registration::is_active)
    }
...
mod tests {
    ...
    #[test]
    fn d4_agent_and_content_validator_roles() {
        let mut reg = VerifierRegistry::new();
        reg.register_agent_operator(addr(20), MIN_REGISTRATION_STAKE, 0)
            .unwrap();
        reg.register_content_validator(addr(21), MIN_REGISTRATION_STAKE, 0)
            .unwrap();
        assert!(reg.is_active_agent_operator(&addr(20)));
