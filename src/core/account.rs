...
impl AccountState {
    ...
    pub fn begin_ai_operator_unbonding(&mut self, address: &Address) -> Result<u64, String> {
        ...
        {
            return Err("AI inference layer operator has open inference or dispute obligations".into());
        }
    ...
    pub fn withdraw_ai_operator(&mut self, address: &Address, fee: u64) -> Result<u64, String> {
        ...
        let final_balance = self
            ...
            .ok_or_else(|| "AI inference layer withdrawal balance overflow/fee underflow".to_string())?;
        let withdrawn = self
            .registry
        ...
        match tx.tx_type {
            ...
            TransactionType::AiOperatorBond => {
                ...
                if tx.to != Address::zero() || !tx.data.is_empty() {
                    return Err("AI inference layer operator bond requires zero recipient and empty data".into());
                }
            ...
            TransactionType::AiOperatorUnbond => {
                if tx.amount != 0 || tx.to != Address::zero() || !tx.data.is_empty() {
                    return Err("AI inference layer unbond requires zero amount/recipient and empty data".into());
                }
            ...
            | TransactionType::AiAttachExecutionProof { .. } => {
                if !self.registry.is_active(&tx.from, roles::AI_OPERATOR) {
                    return Err(
                        "Inference result/proof signer is not an active bonded AI_OPERATOR"
                            .into(),
                    );
                }
            }
            TransactionType::AiDisputeSlash {
                ...
                if !self.ai_registry.is_disputable(
                    ...
                    return Err("No live AI inference layer equivocation evidence for target".into());
                }
            }
            _ => {}
        }
