...
pub enum TransactionType {
    ...
    /// Submit a the AI inference layer inference result by an active bonded RoleId(8) operator.
...
impl Transaction {
    ...
    pub fn new_ai_inference_operator_bond(
        from: Address,
        amount: u64,
        fee: u64,
        nonce: u64,
    ...
    pub fn new_ai_inference_operator_unbond(
        from: Address,
        fee: u64,
        nonce: u64,
        chain_id: u64,
    ...
    pub fn new_ai_inference_operator_withdraw(
        from: Address,
        fee: u64,
        nonce: u64,
        chain_id: u64,
    ...
    pub fn is_valid(&self) -> bool {
        ...
        match &self.tx_type {
            ...
            TransactionType::AiInferenceOperatorBond => {
                if self.amount == 0 || self.to != Address::zero() || !self.data.is_empty() {
                    debug!(
                        "the AI inference layer operator bond requires amount > 0, zero recipient, and empty data"
                    );
                    return false;
                }
            }
            ...
            | TransactionType::AiInferenceOperatorWithdraw => {
                if self.amount != 0 || self.to != Address::zero() || !self.data.is_empty() {
                    debug!(
                        "the AI inference layer unbond/withdraw requires zero amount, zero recipient, and empty data"
                    );
                    return false;
                }
            }
    ...
    pub fn estimate_gas_with_schedule(&self, schedule: GasSchedule) -> u64 {
        let intrinsic = match &self.tx_type {
            ...
            TransactionType::AiInferenceOperatorBond
            | TransactionType::AiInferenceOperatorUnbond
            | TransactionType::AiInferenceOperatorWithdraw => schedule.stake_gas,
        };
...
mod tests {
    ...
    #[test]
    fn ai_inference_operator_bond_signature_commits_amount_and_opcode() {
        let keypair = KeyPair::generate().unwrap();
        let operator = Address::from(keypair.public_key_bytes());
        let mut tx =
            Transaction::new_ai_inference_operator_bond(operator, 1_000, 1, 0, DEFAULT_CHAIN_ID);
        tx.sign(&keypair);
        assert!(tx.verify());
        assert!(tx.is_valid());
...
fn transaction_type_tag(tx_type: &TransactionType) -> u8 {
    match tx_type {
        ...
        TransactionType::AiInferenceOperatorBond => 39,
        TransactionType::AiInferenceOperatorUnbond => 40,
        TransactionType::AiInferenceOperatorWithdraw => 41,
