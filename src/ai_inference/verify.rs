...
/// Verifies a the AI inference layer inference proof with a real plonky3 STARK (verify only).
pub fn verify_inference_stark(
    proof_bytes: &[u8],
    expected_inputs: &ExecutionPublicInputs,
    program: &[u64],
) -> Result<(), String> {
    let envelope: ProofEnvelope = bincode::deserialize(proof_bytes).map_err(|e| {
        format!("the AI inference layer STARK: ProofEnvelope deserialize failed: {e}")
    })?;
    DefaultAdapter::verify(&envelope, expected_inputs, program)
        .map_err(|e| format!("the AI inference layer STARK: verification failed: {e:?}"))
}
...
/// Produces and verifies a the AI inference layer inference proof: a real plonky3 STARK prove/verify round trip.
///
...
pub fn generate_and_verify_proof(
    ...
    if !receipt.success {
        return Err("the AI inference layer STARK: program execution failed".into());
    }
    let pi = build_public_inputs(vm, program, chain_id);
    let envelope = DefaultAdapter::prove(&vm.trace, &pi, program)
        .map_err(|e| format!("the AI inference layer STARK: prove failed: {e:?}"))?;
    DefaultAdapter::verify(&envelope, &pi, program)
        .map_err(|e| format!("the AI inference layer STARK: verify failed: {e:?}"))?;
    Ok(envelope)
}
...
mod tests {
    ...
    #[test]
    fn ai_inference_stark_prove_and_verify_roundtrip() {
