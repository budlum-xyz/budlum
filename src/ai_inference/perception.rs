//! What a the AI inference layer model is allowed to read, and in what form.
//!
//! AI inference layer already answers "may this operator touch this data" through Pollen.
...
//! # Reading only
//!
//! AI inference layer does not generate images or video, and nothing here opens a door to
...
//! `ai_inference::admit_inference_request` on both executor paths.
...
pub enum PerceptionKind {
    ...
    /// Read video frames.
    ///
    /// Reading only. the AI inference layer does not produce video, and the absence of a
...
impl PerceptionRequest {
    ...
    pub fn perception_commitment_tag(&self) -> [u8; 32] {
        hash_fields_bytes(&[
            b"BDLM_AI_INFERENCE_PERCEPTION_V1",
            &self.asset_id.0,
            &self.content_id.0,
...
mod tests {
    ...
    fn reading_modalities_have_no_generating_counterpart() {
        ...
        // AI inference layer reads; it does not generate images or video. A generation
