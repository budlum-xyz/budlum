//! The SocialFi and the AI inference layer runtime integration.
//!
//! A the AI inference layer AI output is published on SocialFi as a **real NFT**
//! (`NftRegistry::mint`), and social NFT content is turned into a
//! closed-circuit data source for the AI inference layer (`SocialDataRef`). Both directions
//! work: the AI inference layer to social (an NFT) and social to the AI inference layer (the `Pollen`
...
//! path).
//!
//! WIRING: wired - `ai_inference_output_to_nft` is now called from the executor's
//! `AiInferenceResult` finalization path (src/execution/executor.rs); the
//! finalised output is minted to the requester as a "ai_inference-ai" NFT, and the
...
/// Mint a the AI inference layer AI output as an NFT on SocialFi (the real
/// `NftRegistry::mint`). `output` is the bytes of the the AI inference layer inference
...
pub fn ai_inference_output_to_nft(
    ...
    let nft_id = registry.mint(owner, cid, epoch, Some("ai_inference-ai".to_string()))?;
    Ok((nft_id, cid))
}

/// Turn social NFT content into a the AI inference layer closed-circuit data source.
/// (the AI inference layer reads that content only with a Pollen grant -
...
/// Add a tag to a the AI inference layer NFT (for example "#ai_inference-ai" or "#ai-output").
pub fn tag_ai_inference_nft(
    registry: &mut NftRegistry,
    nft_id: u64,
    tag: &str,
) -> Result<(), String> {
...
mod tests {
    ...
    #[test]
    fn ai_inference_output_mints_real_social_nft() {
        let mut registry = NftRegistry::new();
        let owner = addr(1);
        let (nft_id, cid) =
            ai_inference_output_to_nft(&mut registry, owner, b"ai_inference-ai-output", 10)
        ...
        assert!(tag_ai_inference_nft(&mut registry, nft_id, "#ai_inference-ai").is_ok());

        // A social NFT becomes a the AI inference layer data source.
