...
//!   (B.U.D. storage), social-data ref (SocialFi ↔ AI inference layer).
//!
//! The operator role is bound to the AI-layer verifier stake (independent of the PoS validator,
//! Composable). Verifier-registry'de `AI_OPERATOR` (RoleId(8)) mapping'i,
...
/// Smallest compute-bond an AI inference layer operator may register with.
///
...
/// Register an AI inference layer operator: the compute bond is the AiRegistry verifier stake.
...
pub fn register_operator(
    ...
    if bond < MIN_OPERATOR_BOND {
        return Err(format!(
            "AI inference layer: compute-bond {bond} is below the minimum {MIN_OPERATOR_BOND}"
        ));
    }
...
/// Whether the operator may take AI inference layer traffic (bond > 0).
#[must_use]
...
/// Is an `AccessGrant` usable for an AI inference layer inference right now?
///
...
///
/// What stays here is the AI inference layer-facing wording of the refusal. An operator
...
pub fn validate_inference_grant(
    ...
    if grant.grantee != *consumer {
        return Err("AI inference layer: grant not issued to this consumer".into());
    }
    if grant.status != AccessGrantStatus::Active {
        return Err("AI inference layer: grant not active".into());
    }
    if now_block > grant.expires_at_block {
        return Err("AI inference layer: grant expired".into());
    }
    if grant.reads_used >= grant.max_reads {
        return Err("AI inference layer: grant read quota exhausted".into());
    }
    ...
    Err("AI inference layer: grant refused by Pollen".into())
}
...
impl TrainingDataGrant {
    ...
    pub fn consume_epoch(&mut self) -> Result<(), String> {
        if self.epochs_used >= self.max_epochs {
            return Err("AI inference layer: training-data grant epochs exhausted".into());
        }
        self.epochs_used += 1;
        Ok(())
    }
...
// SocialFi hardening: social content as an AI inference layer data source

/// An AI inference layer data reference from SocialFi NFT content (expects a Pollen grant).
/// Closed loop: AI inference layer reads social content only with a Pollen grant.
#[derive(Clone, Debug)]
...
impl SocialDataRef {
    /// Produce an AI inference layer data reference from social NFT content.
    #[must_use]
...
/// Build a closed-loop Pollen AccessGrant for an AI inference layer inference.
///
...
#[must_use]
pub fn build_ai_inference_grant(
...
mod tests {
    ...
    #[test]
    fn ai_agrees_with_pollen_on_every_grant_state() {
        ...
        for (i, grant) in cases.iter().enumerate() {
            for now in [0u64, 1, 500, 1001] {
                for consumer in [addr(3), addr(9)] {
                    ...
                    assert_eq!(
                        pollen, ai_inference,
                        "case {i} at block {now}: Pollen says {pollen}, AI inference layer says {ai_inference}"
                    );
                }
            }
        }
    ...
    #[test]
    fn ai_e2e_model_bond_tx_integration() {
        use crate::ai::AiRegistry;
        ...
        // Model kaydet.
        let model_id = super::inference::register_ai_model(&mut registry, owner, model_hash)
        ...
        // Build the AI inference layer transaction.
        ...
        let tx = super::executor::build_ai_transaction(
            owner,
            operator,
            model_id,
