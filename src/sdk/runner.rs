...
impl BudlRunner {
    ...
    pub fn discover_contracts(&self) -> Result<Vec<BudlContract>, RunnerError> {
        if !self.contracts_dir.exists() {
            return Err(RunnerError::ContractsDirNotFound(
    ...
    pub fn generate_fixtures(&self, seed: u64) -> Result<FixtureOutput, RunnerError> {
...
mod tests {
    ...
    fn runner_generate_fixtures() {
        ...
        assert!(dir
            .path()
            .join("fixtures/pollen_asset_fixtures.json")
            .exists());
        assert!(dir
            .path()
            .join("fixtures/pollen_grant_fixtures.json")
            .exists());
        assert!(dir
            .path()
            .join("fixtures/relayer_intent_fixtures.json")
            .exists());
    }
