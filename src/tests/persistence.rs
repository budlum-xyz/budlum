...
// ---: equivocation/slashing history round-trip -----------------------
...
// ---: silent-error pattern hardening ---------------------------------
...
/// Fix), `try_to_bytes` must return `Ok` and round-trip losslessly.
#[test]
fn try_to_bytes_ok_and_roundtrips_with_populated_registry() {
    ...
    // Pre- due to the tuple-key map; try_to_bytes surfaces any failure).
    let bytes = v2
        .try_to_bytes()
...
/// Genuinely fails on a non-string map key. such a failure became
