...
mod settlement_prod_tests {
    ...
    // The legacy self-declared `FinalityProof::PoW` variant was
    ...
    // Build a valid BFT proof (real BLS cert + snapshot) bound to a
    ...
    // Build a PoA proof with REAL ed25519 authority signatures over a
    ...
    fn verified_pow_commitment_rejects_unbound_or_inconsistent_work() {
        // With the legacy self-declared proof retired, the only
        ...
        let err = blockchain
            ...
            .unwrap_err();
        assert!(err.contains("PoW header chain is empty"));
        ...
        let err = blockchain
            ...
            .unwrap_err();
        assert!(err.contains("does not bind the commitment"));
        ...
        let err = blockchain
            ...
            .unwrap_err();
        assert!(err.contains("leading zero bits"));
    ...
    fn poa_finality_rejects_non_authority_signer() {
        ...
        let err = blockchain
            .submit_verified_domain_commitment(c, proof)
            .unwrap_err();
        assert!(err.contains("non-authority"));
    }
    ...
    fn settlement_verifies_domain_event_proofs_from_committed_event_root() {
        ...
        for index in 0..3u32 {
            ...
            let message = CrossDomainMessage::new(CrossDomainMessageParams {
                ...
                event_index: index,
                nonce: index as u64,
                sender: test_addr_from_byte(1u8),
                recipient: test_addr_from_byte(2u8),
                payload_hash,
            ...
            event_tree.push(DomainEvent {
                ...
                emitter: test_addr_from_byte(1u8),
    ...
    fn bridge_mint_is_only_called_after_settlement_event_proof_verifies() {
        ...
        let owner = test_addr_from_byte(11u8);
        let recipient = test_addr_from_byte(12u8);
        blockchain
            .bridge_state
        ...
        // (security audit §9) capture the block hash before
    ...
    fn global_header_hash_changes_when_bridge_or_replay_roots_change() {
        ...
        let owner = test_addr_from_byte(21u8);
        let recipient = test_addr_from_byte(22u8);
    ...
    fn bft_finality_rejects_forged_signer_count() {
        ...
        assert!(err.contains("Invalid BFT finality cert") || err.contains("verification failed"));
    }
    ...
    fn bft_finality_rejects_insufficient_quorum() {
        ...
        c.finality_proof_hash = hash_finality_proof(&proof, Address::zero());
        let err = bc.submit_verified_domain_commitment(c, proof).unwrap_err();
        assert!(err.contains("quorum") || err.contains("Invalid BFT finality cert"));
    }
    ...
    // NOTE: the former `zk_finality_accepts_valid_proof_hashes` test was
    ...
    fn zk_finality_rejects_when_no_accepted_proof() {
        // (Option B) a ZK finality request must reference an accepted,
        ...
        c.finality_proof_hash = hash_finality_proof(&proof, Address::zero());
        let err = bc.submit_verified_domain_commitment(c, proof).unwrap_err();
        assert!(err.contains("no accepted ZK proof"));
    }
    ...
    fn attack_bridge_double_lock_same_asset() {
        ...
        let owner = test_addr_from_byte(10u8);
        let recipient = test_addr_from_byte(20u8);
    ...
    fn attack_bridge_replay_root_unchanged_when_mint_status_is_invalid() {
        ...
        let owner = test_addr_from_byte(3u8);
        let recipient = test_addr_from_byte(4u8);
        bc.state.bridge_state.register_asset(asset, 1).unwrap();
