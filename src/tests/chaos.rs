...
mod chaos_tests {
    ...
    fn test_chaos_cross_domain_replay_and_tamper_resistance() {
        ...
        let owner = test_addr_from_byte(7u8);
        let recipient = test_addr_from_byte(8u8);
    ...
    fn test_chaos_domain_event_proof_rejects_wrong_root_and_index() {
        ...
        let owner = test_addr_from_byte(9u8);
        let recipient = test_addr_from_byte(10u8);
    ...
    fn test_chaos_event_proof_rejects_foreign_domain_and_payload_tamper() {
        ...
        let recipient = test_addr_from_byte(32u8);
    ...
    async fn test_chaos_multi_consensus_concurrent_validation_and_settlement() {
        ...
        for i in 1..=num_blocks {
            ...
            let msg = CrossDomainMessage::new(CrossDomainMessageParams {
                ...
                event_index: 0,
                nonce: i,
                sender: test_addr_from_byte(7u8),
                recipient: test_addr_from_byte(8u8),
    ...
    async fn test_chaos_end_to_end_cross_domain_concurrent_transfers() {
        ...
        // (security audit §9) capture the commitment block hash so the
    ...
    fn test_chaos_v2_mempool_poison_flooder_evicted_by_honest_fees() {
        ...
        // Helper: Create signed transaction
        let create_signed_tx = |seed_byte: u8, fee: u64| -> Transaction {
            let mut seed = [0u8; 32];
            seed[0] = seed_byte;
        ...
        let rich = create_signed_tx(0x7F, 3);
