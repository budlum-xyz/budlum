...
/// Role. `begin_ai_inference_operator_unbonding` called `begin_unbonding_with_delay`
...
#[test]
fn ai_inference_operator_unbonding_follows_the_governance_parameter() {
    use crate::registry::RegistryParams;
    ...
    let bond = state.required_ai_inference_bond(crate::core::transaction::DEFAULT_CHAIN_ID);
    state
        .bond_ai_inference_operator(&operator, bond, crate::core::transaction::DEFAULT_CHAIN_ID)
        .expect("bond at the required floor");

    state.epoch_index = 11;
    let release = state
        .begin_ai_inference_operator_unbonding(&operator)
...
#[test]
fn the_ai_inference_bond_is_not_withdrawable_through_the_role_bond_path() {
    ...
    state
        .begin_role_bond_unbonding(&operator, roles::AI_INFERENCE_OPERATOR)
        .expect_err("the RoleId(8) bond has its own unbonding entry point");
    state
        .withdraw_role_bond(&operator, roles::AI_INFERENCE_OPERATOR)
