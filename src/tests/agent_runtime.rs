...
fn model_and_request(requester: Address) -> (AiModelSpec, AiInferenceRequest) {
    ...
    let mut request = AiInferenceRequest {
        ...
        perception: Some(crate::agent::perception::PerceptionRequest {
...
#[test]
fn signed_agent_bond_debits_balance_and_registers_role8() {
    ...
    let tx = Transaction::new_agent_operator_bond(operator, amount, fee, 0, DEFAULT_CHAIN_ID);
    Executor::apply_transaction(&mut state, &tx).expect("valid Agent bond must apply");

    let registration = state
        .registry
        .get(&operator, roles::AGENT_OPERATOR)
        .expect("RoleId(8) registration must exist");
    assert_eq!(registration.stake, amount);
    assert!(state.registry.is_active(&operator, roles::AGENT_OPERATOR));
...
#[test]
fn agent_bond_floor_matches_each_known_network_validator_floor() {
    let state = AccountState::new();
    assert_eq!(
        state.required_agent_bond(
    ...
    assert_eq!(
        state.required_agent_bond(DEFAULT_CHAIN_ID),
        crate::core::chain_config::Network::Devnet.min_stake()
    );
}

#[test]
fn below_floor_agent_bond_is_atomic() {
    let mut state = AccountState::new();
    ...
    let err = Executor::apply_transaction(&mut state, &tx)
        .expect_err("below-floor Agent bond must fail closed");
...
#[test]
fn mainnet_agent_bond_rejects_devnet_sized_principal() {
    let mut state = AccountState::new();
    ...
    let tx = Transaction::new_agent_operator_bond(
        operator,
        amount,
        fee,
        0,
    ...
    let err = Executor::apply_transaction(&mut state, &tx)
        .expect_err("mainnet must reject a devnet-sized Agent bond");
...
#[test]
fn agent_unbond_waits_seven_epochs_and_withdraws_principal() {
    let mut state = AccountState::new();
    let operator = Address::from([0x24; 32]);
    let bond_amount = state.required_agent_bond(DEFAULT_CHAIN_ID);
    let fee = state.base_fee.max(1);
    state.add_balance(&operator, bond_amount + fee * 3);

    let bond =
        Transaction::new_agent_operator_bond(operator, bond_amount, fee, 0, DEFAULT_CHAIN_ID);
    Executor::apply_transaction(&mut state, &bond).expect("bond");
    let unbond = Transaction::new_agent_operator_unbond(operator, fee, 1, DEFAULT_CHAIN_ID);
    ...
    let withdraw = Transaction::new_agent_operator_withdraw(operator, fee, 2, DEFAULT_CHAIN_ID);
...
#[test]
fn unresolved_agent_result_blocks_unbonding() {
    let mut state = AccountState::new();
    ...
    let unbond = Transaction::new_agent_operator_unbond(operator, fee, 2, DEFAULT_CHAIN_ID);
...
#[test]
fn pos_validator_without_agent_bond_cannot_submit_result() {
    let mut state = AccountState::new();
    ...
    let err = Executor::apply_transaction(&mut state, &tx)
        .expect_err("PoS membership must not imply Agent authorization");
    assert!(err.contains("active bonded AGENT_OPERATOR"));
    assert!(!state.ai_registry.results.contains_key(&request.request_id));
    assert_eq!(state.get_balance(&operator), fee);
}

#[test]
fn bonded_agent_operator_can_submit_result() {
    let mut state = AccountState::new();
    ...
    let bond = Transaction::new_agent_operator_bond(operator, amount, fee, 0, DEFAULT_CHAIN_ID);
    Executor::apply_transaction(&mut state, &bond).expect("bond");
...
fn conflicting_signed_result_commits_evidence_and_burns_only_role8_bond() {
    ...
    let unbond = Transaction::new_agent_operator_unbond(operator, fee, 3, DEFAULT_CHAIN_ID);
    ...
    assert_eq!(
        ...
        validator_stake,
        "Agent application evidence must not erase independent PoS stake"
    );
    assert_eq!(
        state.total_bud_committed(),
...
fn the_effort_tier_is_inside_the_request_identity() {
    use crate::agent::effort::EffortTier;
...
#[test]
fn finalized_agent_output_is_bridged_to_socialfi_nft() {
    let mut state = AccountState::new();
    let requester = Address::from([0x51; 32]);
    ...
    // The operators: a AGENT_OPERATOR bond, granting the authority to sign
    ...
    for op in [operator, operator2] {
        ...
        let bond = Transaction::new_agent_operator_bond(op, bond_amount, fee, 0, DEFAULT_CHAIN_ID);
        Executor::apply_transaction(&mut state, &bond).expect("bond");
    }
    ...
    let output = b"agent-finalized-output".to_vec();
