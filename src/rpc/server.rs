...
impl RpcServer {
    ...
    fn ai_inference_readiness_json(
        chain_id: u64,
        active_bonded_operators: usize,
    ) -> serde_json::Value {
...
impl BudlumApiServer for RpcServer {
    ...
    async fn registry_register(
        ...
        // Stake onboards validators; AiInferenceOperatorBond onboards RoleId(8)
        ...
        if !matches!(
            ...
            return Err(ErrorObjectOwned::owned(
                -32602,
                "registry_register requires a Stake or AiInferenceOperatorBond transaction",
                None::<()>,
            ));
        }
        if !tx.verify() {
    ...
    async fn ai_inference_stats(&self) -> Result<serde_json::Value, ErrorObjectOwned> {
        let active_operators = self
            .chain
            .get_registry_active_members(crate::registry::role::roles::AI_INFERENCE_OPERATOR)
            .await;
        let chain_id = self.chain.get_chain_id().await;
        Ok(Self::ai_inference_readiness_json(
            chain_id,
...
mod tests {
    ...
    #[test]
    fn ai_inference_stats_fail_closed_until_runtime_wiring_is_complete() {
        let stats = RpcServer::ai_inference_readiness_json(1, 3);
