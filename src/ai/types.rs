...
pub struct AiModelSpec {
    ...
    /// ([`crate::agent::perception::ModalitySet`]).
    ///
    ...
    #[serde(default = "crate::agent::perception::ModalitySet::text_only")]
    pub modalities: crate::agent::perception::ModalitySet,
}
...
pub struct AiInferenceRequest {
    ...
    #[serde(default)]
    pub perception: Option<crate::agent::perception::PerceptionRequest>,
    /// How much work the requester is paying for.
    ///
    /// `src/agent/effort.rs` promised two rules and could keep neither while
    ...
    #[serde(default)]
    pub effort: crate::agent::effort::EffortTier,
}

/// The canonical commitment over an input ref: `AGENT_INPUT_COMMIT_V1`
/// domain tag + the ref bytes.
///
/// Admission (`agent::admit_inference_request`) verifies that a request's
...
/// paying for new content. The builder (`agent::inference`) uses the same
/// function, so honest requests pass unchanged.
#[must_use]
pub fn canonical_input_commitment(input_ref: &[u8]) -> [u8; 32] {
    let mut hasher = Sha256::new();
    hasher.update(b"AGENT_INPUT_COMMIT_V1");
...
/// AI inference result submitted by an active bonded Agent operator (RoleId 8).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// Callback event recorded when an outcome is finalized
...
/// Dispute status info for a (request, verifier) pair.
...
/// Verifier stake info for a single address.
...
impl AiExecutionPublicInputs {
    ...
    pub const fn from_execution_inputs(pi: &bud_proof::ExecutionPublicInputs) -> Self {
        Self {
            ...
            state_writes_digest: pi.state_writes_digest,
        }
    }
    ...
    pub const fn to_execution_inputs(&self) -> bud_proof::ExecutionPublicInputs {
        bud_proof::ExecutionPublicInputs {
            ...
            state_writes_digest: self.state_writes_digest,
        }
    }
}
...
pub struct AiInferenceOutcome {
    ...
    pub finalized_at_block: u64,
    /// Callback address from the original request.
...
impl AiAgentReputation {
    ...
    pub fn trust_score(&self) -> f64 {
        ...
        (payment_reliability * 0.4 + inference_quality * 0.6).clamp(0.0, 1.0)
    }
...
pub enum AiPaymentEscrowStatus {
    ...
    Reclaimed,
    /// Non-escrowed immediate settlement completed (audit trail retained).
    SettledImmediate,
}

/// Immutable settlement receipt kept after a payment leaves the live escrow map.
