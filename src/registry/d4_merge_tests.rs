...
//!
//! And that `AI_INFERENCE_OPERATOR` (RoleId 8) is preserved (never removed).
//!
...
/// Preserved `AI_INFERENCE_OPERATOR` role. This is the core "merge" assertion.
#[test]
fn d4_single_registry_serves_all_four_domains() {
    ...
    // AI_INFERENCE_OPERATOR must remain preserved (acceptance criterion).
    reg.register_ai_inference_operator(addr(5), MIN_REGISTRATION_STAKE, 0)
        .expect("ai_inference operator registration");
    ...
    assert!(reg.is_active_ai_inference_operator(&addr(5)));
...
/// AI_INFERENCE_OPERATOR (RoleId 8) is preserved across merge, its RoleId is
/// Pinned and its registration path is intact.
#[test]
fn d4_ai_inference_operator_preserved() {
    let mut reg = PermissionlessRegistry::new();
    reg.register_ai_inference_operator(addr(8), MIN_REGISTRATION_STAKE, 0)
        .unwrap();

    assert!(reg.is_active_ai_inference_operator(&addr(8)));
    assert_eq!(roles::AI_INFERENCE_OPERATOR.value(), 8);

    // Slashing the validator role of the same account also jails its ai_inference
    ...
    .unwrap();
    assert!(!reg.is_active_ai_inference_operator(&a));
}
