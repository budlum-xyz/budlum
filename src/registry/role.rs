...
impl std::fmt::Display for RoleId {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match *self {
            ...
            // MASTER_VERIFIER is a documented alias of VERIFIER (both RoleId(2)),
            // So a second arm for it would be unreachable. One arm covers both.
            roles::VERIFIER => write!(f, "master_verifier"),
            ...
            roles::AI_INFERENCE_OPERATOR => write!(f, "ai_inference_operator"),
...
pub mod roles {
    ...
    /// Distinct semantic label matrix. Preserves AI_INFERENCE_OPERATOR=8.
    ...
    /// (see README, "Permissionless" section).
    ...
    /// A the AI inference layer decentralized AI operator: a compute bond, independent of PoS.
    /// Must be preserved acceptance (RoleId 8).
    pub const AI_INFERENCE_OPERATOR: RoleId = RoleId(8);
...
mod tests {
    ...
    fn well_known_roles_render_names() {
        ...
        assert_eq!(
            format!("{}", roles::AI_INFERENCE_OPERATOR),
            "ai_inference_operator"
        );
    ...
    #[test]
    fn ai_inference_operator_role_id_value_is_8() {
    ...
    fn role_id_ordering() {
        ...
        assert!(roles::ATTESTER < roles::AI_INFERENCE_OPERATOR);
    }
}
