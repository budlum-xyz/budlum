//! # Agent - the decentralized AI layer (real budlum-core wiring)
//!
//! A closed-loop AI layer. This module binds Agent to real budlum-core
//! primitives (no mocks):
//!
...
//!   (B.U.D. storage), social-data ref (SocialFi ↔ Agent).
//!
//! The operator role is bound to the AI-layer verifier stake (independent of the PoS validator,
//! Composable). Verifier-registry'de `AGENT_OPERATOR` (RoleId(8)) mapping'i,
...
pub mod inference;
pub mod metrics;
// What a model may read, and in what form. Reading only: Agent does not
...
/// Smallest compute-bond a Agent operator may register with.
///
...
/// Register a Agent operator: the compute bond is the AiRegistry verifier stake.
...
pub fn register_operator(
    ...
    if bond < MIN_OPERATOR_BOND {
        return Err(format!(
            "Agent: compute-bond {bond} is below the minimum {MIN_OPERATOR_BOND}"
        ));
    }
...
/// Whether the operator may take Agent traffic (bond > 0).
#[must_use]
...
pub fn admit_inference_request(
    ...
    let modalities = registry
        .models
        .get(&req.model_id)
        .map_or(crate::agent::perception::ModalitySet::none(), |m| {
            m.modalities
        });
...
/// Is an `AccessGrant` usable for a Agent inference right now?
///
...
///
/// What stays here is the Agent-facing wording of the refusal. An operator
...
pub fn validate_inference_grant(
    ...
    if grant.grantee != *consumer {
        return Err("Agent: grant not issued to this consumer".into());
    }
    if grant.status != AccessGrantStatus::Active {
        return Err("Agent: grant not active".into());
    }
    if now_block > grant.expires_at_block {
        return Err("Agent: grant expired".into());
    }
    if grant.reads_used >= grant.max_reads {
        return Err("Agent: grant read quota exhausted".into());
    }
    ...
    Err("Agent: grant refused by Pollen".into())
}
...
impl TrainingDataGrant {
    ...
    pub fn consume_epoch(&mut self) -> Result<(), String> {
        if self.epochs_used >= self.max_epochs {
            return Err("Agent: training-data grant epochs exhausted".into());
        }
        self.epochs_used += 1;
        Ok(())
    }
...
// SocialFi hardening: social content as a Agent data source

/// A Agent data reference from SocialFi NFT content (expects a Pollen grant).
/// Closed loop: Agent reads social content only with a Pollen grant.
#[derive(Clone, Debug)]
...
impl SocialDataRef {
    /// Produce a Agent data reference from social NFT content.
    #[must_use]
...
/// Build a closed-loop Pollen AccessGrant for a Agent inference.
///
...
#[must_use]
pub fn build_agent_inference_grant(
...
mod tests {
    ...
    #[test]
    fn agent_agrees_with_pollen_on_every_grant_state() {
        ...
        for (i, grant) in cases.iter().enumerate() {
            for now in [0u64, 1, 500, 1001] {
                for consumer in [addr(3), addr(9)] {
                    ...
                    let agent = validate_inference_grant(grant, &consumer, now).is_ok();
                    assert_eq!(
                        pollen, agent,
                        "case {i} at block {now}: Pollen says {pollen}, Agent says {agent}"
                    );
                }
            }
        }
    ...
    /// E2E: model registration + operator bond + agent transaction build -> the tx_type is correct.
    #[test]
    fn agent_e2e_model_bond_tx_integration() {
        use crate::ai::AiRegistry;
        ...
        // Model kaydet.
        let model_id = super::inference::register_agent_model(&mut registry, owner, model_hash)
        ...
        let tx = super::executor::build_agent_transaction(
            owner,
            operator,
            model_id,
            b"agent-e2e-input".to_vec(),
            10,
            100,
            0,
            45262,
    ...
    fn text_perception() -> crate::agent::perception::PerceptionRequest {
        crate::agent::perception::PerceptionRequest {
            asset_id: crate::pollen::AssetId([1; 32]),
            content_id: crate::storage::content_id::ContentId([2; 32]),
            kind: crate::agent::perception::PerceptionKind::Text,
            declared_units: 100,
        }
    }
    ...
    fn text_request(
        model_id: AiModelId,
        perception: Option<crate::agent::perception::PerceptionRequest>,
    ...
    fn admit_rejects_request_without_declaration() {
        let mut registry = AiRegistry::new();
        let model_id =
            super::inference::register_agent_model(&mut registry, Address([1; 32]), [9u8; 32])
                .unwrap();
    ...
    fn admit_rejects_modality_model_did_not_declare() {
        ...
        p.kind = crate::agent::perception::PerceptionKind::Image;
