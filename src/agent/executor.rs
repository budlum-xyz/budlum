...
//!
//! A Agent inference request is carried to the executor as a
...
/// The Agent transaction request to be sent to the executor (the metadata
/// seam).
...
/// Build a Agent inference transaction (the form to send to the executor).
///
...
pub fn build_agent_transaction(
    from: Address,
    to: Address,
    model_id: AiModelId,
    input_data: Vec<u8>,
    ...
    let req = inference::build_agent_request(
        from,
        model_id,
        input_data.clone(),
        max_fee,
...
mod tests {
    ...
    #[test]
    fn build_agent_tx_produces_ai_inference_type() {
        ...
        let tx = build_agent_transaction(
            from,
            Address([2; 32]),
            AiModelId([3; 32]),
            b"input".to_vec(),
    ...
    fn executor_request_from_inference_request() {
        ...
        let exec = AgentExecutorRequest::from_inference_request(&req);
