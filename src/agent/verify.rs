...
/// Verifies a Agent inference proof with a real plonky3 STARK (verify only).
pub fn verify_inference_stark(
    ...
    let envelope: ProofEnvelope = bincode::deserialize(proof_bytes)
        .map_err(|e| format!("Agent STARK: ProofEnvelope deserialize failed: {e}"))?;
    DefaultAdapter::verify(&envelope, expected_inputs, program)
        .map_err(|e| format!("Agent STARK: verification failed: {e:?}"))
}
...
/// Produces and verifies a Agent inference proof: a real plonky3 STARK prove/verify round trip.
///
...
mod tests {
    ...
    #[test]
    fn agent_stark_prove_and_verify_roundtrip() {
