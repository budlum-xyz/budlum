//! Runtime - the Agent inference flow (over the real `AiRegistry`).
//!
//! The end-to-end flow of a Agent query through the real budlum-core AI layer:
...
/// Register a Agent model on chain (AiModelSpec + register_model).
pub fn register_agent_model(
...
/// Build a closed-circuit Agent inference request (with a canonical request_id).
///
...
/// [`crate::agent::validate_inference_grant`] was called from nowhere in
...
pub fn build_agent_request(
    ...
    crate::agent::validate_inference_grant(grant, &requester, submitted_at_block)?;
...
/// Build a Agent inference result (the operator's answer).
pub fn build_agent_result(
    ...
    hasher.update(b"AGENT_OUTPUT_COMMIT_V1");
...
mod tests {
    ...
    /// The end-to-end Agent inference flow over the real AiRegistry.
    #[test]
    fn agent_full_inference_flow_on_real_registry() {
        ...
        let model_id =
            register_agent_model(&mut registry, owner, model_hash).expect("model register");
        ...
        let res = build_agent_result(req_id, operator, b"agent-output".to_vec(), 1, 2)
            .expect("build result");
