...
impl Blockchain {
    ...
    pub fn tx_precheck(&self, tx: &Transaction) -> serde_json::Value {
        ...
        match tx.tx_type {
            ...
            crate::core::transaction::TransactionType::AiInferenceOperatorBond => {
                if tx.amount < self.state.required_ai_inference_bond(tx.chain_id) {
                    reasons.push("ai_inference_operator_bond_below_network_floor".to_string());
                }
                if tx.to != Address::zero() || !tx.data.is_empty() {
                    reasons.push("invalid_ai_inference_operator_bond_shape".to_string());
                }
                if self
                    .state
                    .registry
                    ...
                    .is_some()
                {
                    reasons.push("ai_inference_operator_already_registered".to_string());
                }
            }
            crate::core::transaction::TransactionType::AiInferenceOperatorUnbond => {
                if tx.amount != 0 || tx.to != Address::zero() || !tx.data.is_empty() {
                    reasons.push("invalid_ai_inference_operator_unbond_shape".to_string());
                }
                if !self.state.registry.is_active(
                    &tx.from,
                    crate::registry::role::roles::AI_INFERENCE_OPERATOR,
                ) {
                    reasons.push("ai_inference_operator_not_active".to_string());
                }
                if self
                    .state
                    .ai_registry
                ...
                {
                    reasons.push("ai_inference_operator_has_open_obligations".to_string());
                }
            }
            crate::core::transaction::TransactionType::AiInferenceOperatorWithdraw => {
                if tx.amount != 0 || tx.to != Address::zero() || !tx.data.is_empty() {
                    reasons.push("invalid_ai_inference_operator_withdraw_shape".to_string());
                }
                match self.state.registry.get(
                    &tx.from,
                    crate::registry::role::roles::AI_INFERENCE_OPERATOR,
                ) {
                    Some(registration) => match registration.status {
                        ...
                        crate::registry::MemberStatus::Unbonding { .. } => {
                            reasons.push("ai_inference_bond_still_unbonding".to_string());
                        }
                        _ => reasons.push("ai_inference_operator_not_unbonding".to_string()),
                    },
                    None => reasons.push("ai_inference_operator_not_registered".to_string()),
                }
            }
            ...
            | crate::core::transaction::TransactionType::AiAttachExecutionProof { .. } => {
                if !self.state.registry.is_active(
                    ...
                    reasons.push("ai_inference_operator_unauthorized".to_string());
                }
            }
            ...
            crate::core::transaction::TransactionType::AiDisputeSlash {
                ...
                if !self.state.registry.is_active(
                    ...
                    reasons.push("ai_inference_slash_operator_inactive".to_string());
                }
                if !self.state.ai_registry.is_disputable(
                    ...
                    reasons.push("ai_inference_equivocation_evidence_missing".to_string());
                }
            }
            _ => {}
        }
