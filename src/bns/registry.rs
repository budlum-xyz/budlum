...
impl BnsRegistry {
    ...
    pub fn calculate_cost(&self, name: &str, duration: u64) -> u64 {
        ...
        if multiplier >= 10 || duration >= 100 { base * 2 } else { base }
    }

    pub fn register(&mut self, name: String, owner: Address, current_epoch: u64, duration: u64) -> Result<(), BnsError> {
        if name.len() < 3 || name.len() > 32 { return Err(BnsError::InvalidName); }
        if let Some(record) = self.names.get(&name) {
            if record.expires_at > current_epoch { return Err(BnsError::NameTaken); }
        }
    ...
    pub fn register_subdomain(&mut self, parent_name: &str, sub_label: String, owner: Address, caller: &Address) -> Result<(), BnsError> {
        let parent = self.names.get_mut(parent_name).ok_or(BnsError::InvalidName)?;
        if &parent.owner != caller { return Err(BnsError::NotOwner); }
    ...
    pub fn resolve_subdomain(&self, parent_name: &str, sub_label: &str, current_epoch: u64) -> Option<Address> {
        ...
        if parent.expires_at > current_epoch { parent.subdomains.get(sub_label).cloned() } else { None }
    }
    ...
    pub fn set_content(&mut self, name: &str, owner: &Address, cid: ContentId) -> Result<(), BnsError> {
        ...
        if &record.owner != owner { return Err(BnsError::NotOwner); }
    ...
    pub fn resolve_content(&self, name: &str, current_epoch: u64) -> Option<ContentId> {
        self.names.get(name).and_then(|record| {
            if record.expires_at > current_epoch { record.content_id } else { None }
        })
    }

    pub fn resolve(&self, name: &str, current_epoch: u64) -> Option<Address> {
        self.names.get(name).and_then(|record| {
            if record.expires_at > current_epoch { Some(record.owner) } else { None }
        })
    }
    ...
    pub fn set_storage(&mut self, name: &str, caller: Address, storage_root: [u8; 32], storage_domain_id: u32, current_epoch: u64) -> Result<(), BnsError> {
        let rec = self.names.get_mut(name).ok_or(BnsError::InvalidName)?;
        if rec.owner != caller { return Err(BnsError::NotOwner); }
        if rec.expires_at <= current_epoch { return Err(BnsError::Expired); }
