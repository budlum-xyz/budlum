    ...
    pub fn apply_transaction_checked(
        ...
        match &tx.tx_type {
            ...
            TransactionType::AiOperatorBond => {
                ...
                let sender = state.get_or_create(&tx.from);
                sender.balance = sender.balance.checked_sub(tx.fee).ok_or_else(|| {
                    BudlumError::validation("balance_underflow", "AI inference layer bond fee underflow")
                })?;
                sender.nonce = sender.nonce.saturating_add(1);
            }
            TransactionType::AiOperatorUnbond => {
                ...
                let release_epoch =
                    state
                        ...
                        .map_err(|error| {
                            BudlumError::validation("ai_operator_unbond_failed", error)
                        })?;
                let sender = state.get_or_create(&tx.from);
                sender.balance = sender.balance.checked_sub(tx.fee).ok_or_else(|| {
                    BudlumError::validation("balance_underflow", "AI inference layer unbond fee underflow")
                })?;
                sender.nonce = sender.nonce.saturating_add(1);
                tracing::info!(
                    operator = %tx.from,
            ...
            TransactionType::AiDisputeSlash {
                ...
                let slash = state
                    ...
                    .map_err(|e| {
                        BudlumError::validation("ai_role_slash_failed", e.to_string())
                    })?;
                tracing::warn!(
                    operator = %verifier,
