...
pub enum ThreeNftRegistryError {
    ...
    UnknownPin(u64),
    /// The pin is a production recipe record: it holds no body, so there is
    /// nothing stored to re-emit. Regeneration from the recipe is the serving
    /// validator's job, not a re-emission from a stored body.
    GeneratedHoldsNoBody(u64),
...
impl std::fmt::Display for ThreeNftRegistryError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::GeneratedHoldsNoBody(id) => write!(
                f,
                "three nft registry: pin {id} is a production recipe and holds no body to re-emit"
            ),
...
/// The single durable object B.U.D. 3.0 keeps (§19.4 K15): the tarif record.
///
/// A body is never a separate object. A record either produces its content
/// from a seed ([`RecipeRecord::Generated`], held bytes = 0, generated on demand)
/// or carries the compressed/raw body inside itself ([`RecipeRecord::WithBody`],
/// held bytes = the body length). Re-emission is only meaningful for a
/// with_body record: the body it re-encodes lives in the record; a production
/// record is regenerated by whoever serves it, never re-emitted from a stored
/// body.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum RecipeRecord {
    /// Production recipe (generated content). Holds no body: the bytes are
    /// produced from the recipe on demand and never stored here.
    Generated {
        /// Public pipe parameters the object was minted from.
        recipe: ThreeRecipePublic,
    },
    /// With-body recipe (organic or encrypted content). The compressed or raw
    /// body lives inside the record; this is the `WithBody.body` of the spec.
    WithBody {
        /// Public pipe parameters the object was minted from. These are not
        /// the secret: for a sealed object the seed and the payload key never
        /// live here, and re-emission runs over the body this record holds.
        recipe: ThreeRecipePublic,
        /// The A1 packed container the recipe pins. For a sealed object this
        /// is the encrypted payload; for a public one the plaintext body.
        body: Vec<u8>,
    },
}

impl RecipeRecord {
    /// A with_body record: the common mint path for organic or encrypted
    /// content.
    #[must_use]
    pub const fn with_body(recipe: ThreeRecipePublic, body: Vec<u8>) -> Self {
        Self::WithBody { recipe, body }
    }

    /// A production (body-less) record for generated content.
    #[must_use]
    pub const fn generated(recipe: ThreeRecipePublic) -> Self {
        Self::Generated { recipe }
    }

    /// The rent-accounted byte count (K14b/K15): zero for a production
    /// recipe, the body length for a with_body record. This is the number the
    /// fee model's rent meter runs over.
    #[must_use]
    pub fn held_bytes(&self) -> usize {
        match self {
            Self::Generated { .. } => 0,
            Self::WithBody { body, .. } => body.len(),
        }
    }

    /// The public pipe recipe either kind carries.
    #[must_use]
    pub fn recipe(&self) -> &ThreeRecipePublic {
        match self {
            Self::Generated { recipe } | Self::WithBody { recipe, .. } => recipe,
        }
    }
}

/// One pinned Three object. The row is a single tarif record: the public pipe
/// parameters and, for a with_body record, the body itself. A future reader
/// can rebuild the BDLV stream from a [`RecipeRecord::WithBody`] pin even after
/// every storage validator that held a copy has churned out; a
/// [`RecipeRecord::Generated`] pin holds nothing to rebuild and is regenerated from
/// the recipe by whoever serves it.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PinRow {
    /// Marketplace metadata.
    pub meta: ThreeNftMeta,
    /// The tarif record this pin is (B.U.D. 3.0: a single object).
    pub tarif: RecipeRecord,
}
...
impl ThreeNftRegistry {
    ...
    pub fn drop_validator(&mut self, id: u64) -> Result<(), ThreeNftRegistryError> {
        let row = self
            .rows
            .get_mut(&id)
            .ok_or(ThreeNftRegistryError::UnknownPin(id))?;
    ...
    /// and the body the record holds.
    ///
    /// The re-emission is not a re-encode: it opens the recipe against the
    /// body through [`RecipeEmitter`], rebuilds the carousel drops bit-equal
    /// to the original encode, and wraps them the same way the product encoder
    /// does. The result is therefore the product video itself, so a reader
    /// that recorded the stream id can verify it matches.
    ///
    /// Only a [`RecipeRecord::WithBody`] record holds a body to re-emit; a
    /// production record is refused with [`ThreeNftRegistryError::GeneratedHoldsNoBody`].
    ///
    /// # Errors
    ///
    /// [`ThreeNftRegistryError::UnknownPin`] for an unknown id;
    /// [`ThreeNftRegistryError::GeneratedHoldsNoBody`] for a production record;
    /// [`ThreeNftRegistryError::Reemit`] when the body does not match the
    /// recipe or the stream id; [`ThreeNftRegistryError::Video`] when the
    ...
    pub fn reemit_video_from_pin(&self, id: u64) -> Result<Vec<u8>, ThreeNftRegistryError> {
        let row = self
            ...
            .get(&id)
            .ok_or(ThreeNftRegistryError::UnknownPin(id))?;
        let RecipeRecord::WithBody { recipe, body } = &row.tarif else {
            return Err(ThreeNftRegistryError::GeneratedHoldsNoBody(id));
        };
        let emitter = RecipeEmitter::open(recipe.clone(), body)?;
        let count = oneshot_drop_count(recipe.carousel.k, ONESHOT_REPAIR_PERMILLAGE);
...
mod nft_registry_tests {
    ...
    fn churn_reemit_reproduces_the_product_video_bit_equal() {
        ...
        let id = reg.pin(PinRow {
            meta,
            tarif: RecipeRecord::with_body(enc.pipe.recipe.clone(), enc.pipe.packed.clone()),
        });
    ...
    fn a_packed_body_that_does_not_match_the_recipe_is_refused() {
        ...
        let id = reg.pin(PinRow {
            meta,
            tarif: RecipeRecord::with_body(right.pipe.recipe.clone(), wrong.pipe.packed.clone()),
        });
        assert_eq!(
    ...
    /// A production (generated) record is the body-less kind of the tarif
    /// model: it reports zero held bytes and is refused at re-emit time,
    /// because there is no stored body to re-emit - the content is produced
    /// from the recipe by whoever serves it.
    #[test]
    fn generated_record_holds_zero_bytes_and_is_not_reemitted() {
        let enc = product(b"generated-object-body");
        let meta = ThreeNftMeta::from_recipe(
            &ThreeRecipe::Public(enc.pipe.recipe.clone()),
            PreviewMode::None,
        );
        ...
        let id = reg.pin(PinRow {
            meta,
            tarif: RecipeRecord::generated(enc.pipe.recipe.clone()),
        });
        assert_eq!(reg.get(id).unwrap().tarif.held_bytes(), 0);
        assert_eq!(
            reg.reemit_video_from_pin(id),
            Err(ThreeNftRegistryError::GeneratedHoldsNoBody(id))
        );
    }

    /// A with_body record reports exactly the body length it holds, so the
    /// rent meter (K14b/K15) can run over the `WithBody.body` field and over
    /// nothing else.
    #[test]
    fn govdeli_record_reports_the_body_length_it_holds() {
        let enc = product(b"organic-body-bytes");
        let meta = ThreeNftMeta::from_recipe(
            &ThreeRecipe::Public(enc.pipe.recipe.clone()),
            PreviewMode::None,
        );
        ...
        assert_eq!(
            reg.get(id).unwrap().tarif.held_bytes(),
            enc.pipe.packed.len()
        );
        assert_eq!(reg.get(id).unwrap().tarif.recipe(), &enc.pipe.recipe,);
    }
}
