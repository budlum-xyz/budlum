...
impl std::fmt::Display for ThreeNftRegistryError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::UretimHoldsNoBody(id) => write!(
                f,
...
/// from a seed ([`TarifKaydi::Uretim`], held bytes = 0, generated on demand)
/// or carries the compressed/raw body inside itself ([`TarifKaydi::Govdeli`],
/// held bytes = the body length). Re-emission is only meaningful for a
/// with-body record: the body it re-encodes lives in the record; a production
...
pub enum TarifKaydi {
    ...
    /// body lives inside the record; this is the `Govdeli.govde` of the spec.
    Govdeli {
...
impl TarifKaydi {
    /// A with-body record: the common mint path for organic or encrypted
    /// content.
    #[must_use]
    pub const fn govdeli(recipe: ThreeRecipePublic, govde: Vec<u8>) -> Self {
        Self::Govdeli { recipe, govde }
    }

    /// A production (body-less) record for generated content.
    #[must_use]
    pub const fn uretim(recipe: ThreeRecipePublic) -> Self {
        Self::Uretim { recipe }
    }

    /// The rent-accounted byte count (K14b/K15): zero for a production
    /// recipe, the body length for a with-body record. This is the number the
    /// fee model's rent meter runs over.
    #[must_use]
    pub fn held_bytes(&self) -> usize {
        match self {
            Self::Uretim { .. } => 0,
            Self::Govdeli { govde, .. } => govde.len(),
        }
    }
    ...
    pub fn recipe(&self) -> &ThreeRecipePublic {
        match self {
            Self::Uretim { recipe } | Self::Govdeli { recipe, .. } => recipe,
        }
    }
}

/// One pinned Three object. The row is a single tarif record: the public pipe
/// parameters and, for a with-body record, the body itself. A future reader
/// can rebuild the BDLV stream from a [`TarifKaydi::Govdeli`] pin even after
/// every storage validator that held a copy has churned out; a
/// [`TarifKaydi::Uretim`] pin holds nothing to rebuild and is regenerated from
...
impl ThreeNftRegistry {
    ...
    ///
    /// Only a [`TarifKaydi::Govdeli`] record holds a body to re-emit; a
    /// production record is refused with [`ThreeNftRegistryError::UretimHoldsNoBody`].
    ///
    /// # Errors
    ///
    /// [`ThreeNftRegistryError::UnknownPin`] for an unknown id;
    /// [`ThreeNftRegistryError::UretimHoldsNoBody`] for a production record;
    ...
    pub fn reemit_video_from_pin(&self, id: u64) -> Result<Vec<u8>, ThreeNftRegistryError> {
        let row = self
            ...
            .get(&id)
            .ok_or(ThreeNftRegistryError::UnknownPin(id))?;
        let TarifKaydi::Govdeli { recipe, govde } = &row.tarif else {
            return Err(ThreeNftRegistryError::UretimHoldsNoBody(id));
        };
        let emitter = RecipeEmitter::open(recipe.clone(), govde)?;
...
mod nft_registry_tests {
    ...
    fn churn_reemit_reproduces_the_product_video_bit_equal() {
        ...
        let id = reg.pin(PinRow {
            meta,
            tarif: TarifKaydi::govdeli(enc.pipe.recipe.clone(), enc.pipe.packed.clone()),
        });
    ...
    fn a_packed_body_that_does_not_match_the_recipe_is_refused() {
        ...
        let id = reg.pin(PinRow {
            meta,
            tarif: TarifKaydi::govdeli(right.pipe.recipe.clone(), wrong.pipe.packed.clone()),
        });
        assert_eq!(
    ...
    #[test]
    fn uretim_record_holds_zero_bytes_and_is_not_reemitted() {
        ...
        let id = reg.pin(PinRow {
            meta,
            tarif: TarifKaydi::uretim(enc.pipe.recipe.clone()),
        });
        assert_eq!(reg.get(id).unwrap().tarif.held_bytes(), 0);
        assert_eq!(
            reg.reemit_video_from_pin(id),
            Err(ThreeNftRegistryError::UretimHoldsNoBody(id))
        );
    }

    /// A with-body record reports exactly the body length it holds, so the
    /// rent meter (K14b/K15) can run over the `Govdeli.govde` field and over
    /// nothing else.
    #[test]
