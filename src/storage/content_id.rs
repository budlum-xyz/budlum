//! B.U.D. content addressing (Task 0.38, Görev 2 + Task 0.39, vision §8.2).
//!
...
//! wired into `budlum-core` (it lives in BudZKVM), so for Task 0.38 we use the
...
//!
//! Per Task 0.39 plan §0.5 (data-sovereignty / team-independence rule),
...
impl ContentId {
    ...
    /// range (used by Görev 5's `RetrievalChallenge` to pin a deterministic
    /// sub-range within a chunk — vision §8.3 / Task 0.39 §2.5).
    ///
    /// **Critically (Task 0.39 plan §2.5):** the resulting `ContentId` is
