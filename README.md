...
The Universal Settlement Layer is the common ground beneath heterogeneous networks: it does
not ask which consensus produced a fact, only whether the finality proof for that fact holds,
...
| **The quantum horizon.** Ed25519 and ECDSA are expected to be breakable within the lifetime of a chain launched today. | Hybrid finality: BLS12-381 aggregate signatures alongside a post-quantum scheme (ML-DSA-65 by default - FIPS 204 NIST final; legacy Dilithium5 available only for migration). |
...
flowchart TB
  ...
  subgraph USL["Universal Settlement Layer"]
    CONS["Heterogeneous Consensus engines<br/><code>src/consensus/</code>"]
  ...
  GBH --> AI["the AI inference layer AI layer<br/>model registry · compute bond"]
```
...
2. **the AI inference layer grading loop.** `ai-inference-ops tune data.jsonl` validates the dataset
   label (model target, sample count), attaches the curriculum's eval-set
   digest to the plan and checks the binding; `ai-inference-ops eval data.jsonl
   ...
   `ai-inference-serve` refuses a consensus bridge whose residency plan streams more
...
| **the AI inference layer** | [`src/ai_inference/`](src/ai_inference) | Closed-circuit AI layer: model registry, operator compute-bond, effort tiers, Pollen-gated data access, perception declarations (V3), SocialFi output bridge. Off-chain workspace: [`crates/ai-inference/`](crates/ai-inference) |
...
### Root files kept at the root

| File | Why it stays at the root |
| --- | --- |
| `Cargo.toml` · `Cargo.lock` · `build.rs` | Cargo package root: the core crate, its benches and the path dependencies (`budzero/*`, `crates/*`) resolve from here |
| `rust-toolchain.toml` | rustup walks up from the working directory and must find it here |
...
- **Rust 1.97.1**: pinned in [`rust-toolchain.toml`](rust-toolchain.toml); `rustup` selects it automatically
...
cargo build --release              # the core node
cargo test --lib                   # the core test suite
...
[SPECIFICATION.md § 3.3](docs/SPECIFICATION.md).
...
- **`fmt` and `clippy` with `-D warnings`** against the pinned 1.97.1 toolchain, plus a
...
Report vulnerabilities **privately**: see [SECURITY.md](docs/SECURITY.md). Please do not open a
...
**Implemented and tested:** Heterogeneous Consensus on the Universal Settlement Layer (PoW/PoS/PoA/BFT) · BLS + post-quantum hybrid
...
data marketplace · SocialFi primitives · the AI inference layer AI inference layer · EVM chain adapter
...
| [PROVENANCE_NOTES.md](docs/PROVENANCE_NOTES.md) | Per-module record of what each non-trivial implementation is based on |
...
Contributions are welcome. Read [CONTRIBUTING.md](docs/CONTRIBUTING.md) first, it sets a higher
