...
This is a clean-room curation layer, not imported upstream code.  It records
which projects listed by fsf.org / the Free Software Directory are useful to
Budlum modules and, crucially, which license boundary is allowed before anyone
copies source into this PolyForm-Shield tree.

The generated document is intentionally deterministic so CI or a reviewer can
see when the project fit matrix changed.
"""
...
def render() -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            "This document is generated by `tools/fsf_project_fit.py`. It is a clean-room",
            "curation of FSF/Free Software Directory project ideas against Budlum modules; it",
            "does **not** copy upstream project code. Budlum is currently licensed under",
            "PolyForm Shield, so GPL/AGPL source imports are treated as design-only unless a",
            "separate legal/provenance decision creates an explicit boundary.",
            "",
            "Sources inspected:",
            ...
            *rows,
            ...
            "repo-local guardrail for future work. The strongest code candidates for later",
            "Budlum changes are Matrix/Pump.io-style federation event shapes, Argos-style",
            "offline i18n provider seams, and FOSSology/licenseutils-style compliance",
            "gates. GPL/AGPL projects such as GNU Taler are useful for architecture but not",
            "for direct source import into the current Budlum tree.",
            "",
        ]
    )
