...
Status document for the the AI inference layer AI layer. It exists because the README and
...
| SocialFi bridge: finalized the AI inference layer output minted as requester-owned NFT | working (best-effort) | `src/execution/executor.rs` → `ai_inference::social::ai_inference_output_to_nft` |
...
`ai_inference-ai` NFT owned by the requester (best-effort: a mint refusal - e.g. a
...
Every the AI inference layer surface was checked against live code with call-site evidence:
