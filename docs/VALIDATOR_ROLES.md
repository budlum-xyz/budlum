...
## 3. Agent compute: optional

`roles::AGENT_OPERATOR` (RoleId 8). Supplying CPU/GPU capacity for AI inference
...
- a hardware fault that makes an operator miss retrieval challenges slashes the
  storage bond only;
- equivocating on an inference result touches the Agent bond only;
...
| consensus + storage + compute | VALIDATOR, STORAGE_OPERATOR, AGENT_OPERATOR |
| storage only (no block production) | STORAGE_OPERATOR |
| compute only | AGENT_OPERATOR |
...
Agent's costs are settled the same way consensus rewards are: they accrue to
...
Agent compute is not uniform: an operator answers with the machine it owns.
`src/agent/effort.rs` makes that explicit through effort tiers (`0.5x` … `10x`),
...
particular, **without hardware capable of `10.0x`, no verifier can run Agent at
