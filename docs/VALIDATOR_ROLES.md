...
## 3. the AI inference layer compute: optional

`roles::AI_INFERENCE_OPERATOR` (RoleId 8). Supplying CPU/GPU capacity for AI inference
...
- a hardware fault that makes an operator miss retrieval challenges slashes the
  storage bond only;
- equivocating on an inference result touches the the AI inference layer bond only;
...
| consensus + storage + compute | VALIDATOR, STORAGE_OPERATOR, AI_INFERENCE_OPERATOR |
| storage only (no block production) | STORAGE_OPERATOR |
| compute only | AI_INFERENCE_OPERATOR |
...
the AI inference layer's costs are settled the same way consensus rewards are: they accrue to
...
AI inference layer compute is not uniform: an operator answers with the machine it owns.
`src/ai_inference/effort.rs` makes that explicit through effort tiers (`0.5x` … `10x`),
...
particular, **without hardware capable of `10.0x`, no verifier can run the AI inference layer at
