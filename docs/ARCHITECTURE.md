...
stateDiagram-v2
  ...
  note right of Active
    ...
    ATTESTER AI_OPERATOR CONTENT_VALIDATOR
...
flowchart TD
  ...
  subgraph sg2["Role 2: AI inference layer CPU/System Provider"]
    Active --> AiBond[AI_OPERATOR role bond]
    AiBond --> AiCompute[CPU/GPU compute for AI inference]
    AiCompute --> AiServe[Serve AI inference requests]
    AiServe --> AiReward[Inference service reward]
    AiServe --> AiSlash[Compute fault -> slash]
  end
...
| `src/ai_inference/verify.rs` | `build_public_inputs` on the AI inference layer STARK path |
