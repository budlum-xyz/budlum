...
### 2.2 Validator Fee Income
Validators receive no block or epoch emission. For each included transaction, the producer is credited exactly once with `tx.fee - metabolic_burn`. The signed `max_fee` and `priority_fee` fields are retained only for wire compatibility and must be zero/equal to the flat fee until a versioned fee-market migration is approved.
