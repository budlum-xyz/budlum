...
impl DiffusionPrompt {
    pub fn ratio(&self) -> f64 {
        ...
        self.original_size as f64 / (self.prompt.len() as f64 + 32.0 + 32.0 + 32.0)
        // prompt + seed + tarif_hash + commitment
    }
}
...
impl UltraGates {
    pub fn k_bud_optical_ultra(p: &DiffusionPrompt) -> Result<(), &'static str> {
        if p.ratio() < 1000.0 {
            return Err("K-BUD-OPTICAL-ULTRA: ratio <1000 not revolutionary");
        }
        Ok(())
    }
    pub fn k_bud_code_tarif(t: &CodeTarif) -> Result<(), &'static str> {
        if t.ratio() < 100.0 {
            return Err("K-BUD-CODE-TARIF: ratio <100 not revolutionary");
        }
        Ok(())
    }
    pub fn k_bud_log_4layer(l: &Log4Layer) -> Result<(), &'static str> {
        if l.total_ratio() < 1000.0 {
            return Err("K-BUD-LOG-4LAYER: total <1000 not revolutionary");
        }
        Ok(())
    }
}
...
mod tests {
    ...
    #[test]
    fn diffusion_50000x() {
        let p = DiffusionPrompt {
            prompt: "a cat 1920x1080 photorealistic".into(),
            width: 1920,
            height: 1080,
    ...
    fn code_tarif_1000x() {
        let t = CodeTarif {
            tarif: "generate CRUD API with auth".into(),
            seed: [0u8; 32],
